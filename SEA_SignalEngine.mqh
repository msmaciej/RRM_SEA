...
public:
   ...
   //   out_green     = true when fast histogram aligned with main histogram (healthy momentum)
   //                   false when fast has reversed = overbought/oversold signal
   ...
   bool ComputeDPIMainHist(int v_shift, double &out_hist_cur, double &out_hist_prev, bool &out_green)
   {
      ...
      double alphaS     = 2.0 / (double)(S     + 1);
      double alphaU     = 2.0 / (double)(U     + 1);
      double alphaFastR = 2.0 / (double)(FastR + 1);
      double alphaFastS = 2.0 / (double)(FastS + 1);
      ...
      for(int i = bars_needed - 1; i >= v_shift; i--)
      {
         ...
         fe1m = alphaFastR * mom     + (1.0 - alphaFastR) * fe1m;
         fe1a = alphaFastR * abs_mom + (1.0 - alphaFastR) * fe1a;
         fe2m = alphaFastS * fe1m    + (1.0 - alphaFastS) * fe2m;
         fe2a = alphaFastS * fe1a    + (1.0 - alphaFastS) * fe2a;
         fast_line = (fe2a != 0.0) ? (fe2m / fe2a) : 0.0;
      ...
      // out_green = true when fast histogram aligned with main histogram (healthy momentum)
      // out_green = false when fast has reversed = overbought/oversold signal
      double main_hist   = main_line - sig;
      double nested_hist = fast_line - main_line;
      out_green = (main_hist != 0.0 && nested_hist != 0.0 &&
                   (nested_hist * main_hist > 0.0) &&

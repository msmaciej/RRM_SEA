...
int OnInit() {
   ...
   Print("Reversal Logic: ", Settings.CloseOnReverse ? "ON (Reversal)" : "OFF (Fixed Targets)");
   Print("------------------------------------------");
...
🚀 Update Summary: SimpleEA v1.02.012g
I have implemented the specific behaviors for the missing presets based on their strategic intent:
1. PRESET_TREND_SCALP ("The Sniper")
    * Philosophy: Enter only on strong momentum, get in, take profit, get out.
    * Logic: Disables CloseOnReverse. Uses Fixed SL/TP.
    * Votes: Requires 3 votes (Confluence).
    * Indicators: Activates MACD (Momentum) + ADX (Strength) + EMA (Trend).
    * Safety: Tight Stop Loss (1.5x ATR).
2. PRESET_RANGE_GRID ("The Conservative")
    * Philosophy: Buy dips in uptrends (Mean Reversion) with high safety.
    * Logic: Uses Pullback Indicators (Bollinger Lower Band, Stochastic).
    * Votes: Requires 4 votes (High Consensus).
    * Filters: Enables HTF Filter (only trade in direction of H4 trend) and Time Filters.

📁 File: SimpleEA_v1-02-012g.mq5

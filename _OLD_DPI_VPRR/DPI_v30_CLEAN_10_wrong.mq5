                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // Calculate RED/YELLOW extent: use MAXIMUM distance from zero
      // This ensures ribbon fills completely with no gaps on either side
      double redy_extent;
      if(both_above_zero || g_BlueCore[i] > 0.0)
      {
         // Positive side: use the larger value
         redy_extent = MathMax(g_BlueCore[i], hist);
      }
      else if(both_below_zero || g_BlueCore[i] < 0.0)
      {
         // Negative side: use the more negative value
      ...
      if(green_condition)
      {
         ...
         // RED/YELLOW histogram: Extends to furthest point (Blue or hist)
         if(use_yellow)
         {
      ...
      else
      {
         ...
         // RED/YELLOW histogram: Extends to furthest point
         if(use_yellow)
         {
            g_HistPos[i] = redy_extent;
            g_HistNeg[i] = EMPTY_VALUE;

...
//+------------------------------------------------------------------+
//|                      DPI_v31_CLEAN_FIXED.mq5                      |
//|   Purpose: Complete MT4 DPI replica - CORRECTED CCI LOGIC        |
//|   Version: 31 FIXED - Opposite sides color bug fixed             |
//|                                                                  |
//|   BUG FIX:                                                       |
//|   When Blue and hist on OPPOSITE sides, each side must get      |
//|   its color based on WHICH LINE is on that side:                |
//|     - If Blue is positive → color based on hist CCI logic       |
//|     - If hist is positive → color based on hist CCI logic       |
//|     - Each side colored INDEPENDENTLY                            |
//|                                                                  |
//|   The previous version colored both sides based on hist's sign,  |
//|   which was incorrect when they were on opposite sides.          |
//+------------------------------------------------------------------+
#property strict
#property version   "31.10"
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // CORRECTED CCI COLOR LOGIC
      // The histogram color rules are:
      // - When hist >= 0: default YELLOW, CCI < 0 forces RED
      // - When hist < 0:  default RED, CCI >= 0 forces YELLOW
      //
      // This determines the color REGARDLESS of which side hist is on
      ...
      // Now draw the base layer histograms
      // KEY FIX: Use hist_wants_yellow for ALL sides
      // The histogram determines the color, not which line is where
      
      if(opposite_sides)
      {
         // OPPOSITE SIDES: Both positive and negative filled
         // But SAME color logic applies to both (based on hist)
         ...
         // Negative side - SAME color as positive (based on hist)
...
//
// The histogram (red contour = Blue - Red) determines the color:
//   hist >= 0: Yellow (unless CCI < 0, then Red)
//   hist < 0:  Red (unless CCI >= 0, then Yellow)
//
// This color applies to BOTH sides when opposite, because the
// histogram is a single semantic entity - it's just that its
// EXTENT is split across the zero line.
//
// Example:
//   Blue = 0.5, hist = -0.3, CCI = 100
//   hist < 0 AND CCI > 0 → YELLOW override
//   Both positive extent (0.5) and negative extent (-0.3) are YELLOW
//
// This matches MT4 DPI behavior where the histogram ribbon is
// one continuous colored region, even when split by zero.
//
//+------------------------------------------------------------------+

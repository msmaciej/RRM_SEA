...
//+------------------------------------------------------------------+
//|                         DPI_RedLine_Test_MACDCore.mq5            |
//|   Purpose: test which red DPI line candidate matches MT4 DPI     |
//|   No EX4 decompilation. Behaviour comparison helper only.        |
//|   MODIFIED: Added red contour line from v18                      |
//+------------------------------------------------------------------+
#property strict
...
// Plot 9: RED CONTOUR LINE (new from v18)
...
double g_HistPos[];
double g_HistNeg[];
double g_RedContour[];  // NEW: red contour line
...
int OnInit()
{
      ...
      {
            Print("DPI red-line test error: Slow EMA must be > Fast EMA.");
      ...
      SetIndexBuffer(9, g_RedContour,    INDICATOR_DATA);  // NEW
      ...
      ArraySetAsSeries(g_RedContour,    true);  // NEW
      ...
      IndicatorSetString(INDICATOR_SHORTNAME, "DPI RedLine Test MACDCore + Contour");
                        ...
                        const int &spread[])
{
      ...
      g_RedContour[oldest] = EMPTY_VALUE;  // NEW
      ...
      for(int i = rates_total - 2; i >= 0; --i)
      {
            ...
            g_HistGrey[i] = hist;
            g_RedContour[i] = hist;  // NEW: red contour follows histogram value

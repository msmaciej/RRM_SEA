                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      if(green_condition)
      {
         ...
         // GREEN histogram: From 0-line to Red FOLLOW line
         g_HistGreen[i] = g_RedSignal[i];
         
         // RED/YELLOW histogram: Shows gap between Red Follow and Blue Lead
         // This is the standard histogram value (Blue - Red)
         if(use_yellow)
         {
            g_HistPos[i] = hist;
      ...
      else
      {
         ...
         // RED/YELLOW histogram: Standard value (Blue - Red)
         if(use_yellow)
         {
            g_HistPos[i] = hist;
            g_HistNeg[i] = EMPTY_VALUE;

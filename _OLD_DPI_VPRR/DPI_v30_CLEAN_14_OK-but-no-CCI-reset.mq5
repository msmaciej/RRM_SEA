...
//+------------------------------------------------------------------+
//|                       DPI_v30_PERFECT.mq5                         |
//|   Purpose: MT4 DPI replica - PERFECT RIBBON                      |
//|   Version: 30 PERFECT - Complete and accurate ribbon logic       |
//|                                                                  |
//|   RIBBON LOGIC - FINAL CORRECT VERSION:                          |
//|                                                                  |
//|   1. Base RED/YELLOW layer: Always fills from 0 to furthest     |
//|      extent (max on positive, min on negative)                   |
//|                                                                  |
//|   2. GREEN overlay: Only when both same side, fills from 0 to   |
//|      the SMALLER magnitude (closer to zero)                      |
//|      - If both positive: GREEN from 0 to min(Blue, hist)        |
//|      - If both negative: GREEN from 0 to max(Blue, hist)        |
//|                                                                  |
//|   This ensures GREEN never extends beyond Blue line while       |
//|   maintaining the continuous ribbon fill.                        |
//+------------------------------------------------------------------+
#property strict
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // PERFECT RIBBON LOGIC
      //
      // Base layer (RED/YELLOW): Fills to FURTHEST extent
      // GREEN overlay: Fills to CLOSER TO ZERO (never beyond Blue line)
      ...
      // Calculate GREEN extent - should never go beyond Blue line
      double green_extent = 0.0;
      if(green_condition)
      {
         if(both_above_zero)
         {
            // Both positive: GREEN goes to the SMALLER value (closer to zero)
            green_extent = MathMin(g_BlueCore[i], hist);
         }
         else if(both_below_zero)
         {
            // Both negative: GREEN goes to the LARGER value (closer to zero)
            green_extent = MathMax(g_BlueCore[i], hist);
         }
      }
...
//+------------------------------------------------------------------+
//
// PERFECT RIBBON LOGIC - FINAL VERSION
//
// The ribbon has two layers:
//
// 1. BASE LAYER (RED/YELLOW):
//    Always fills from 0 to the FURTHEST extent
//    - Positive side: max(Blue, hist)
//    - Negative side: min(Blue, hist)
//    - When on opposite sides: fills BOTH sides
//
// 2. GREEN OVERLAY:
//    Only when both same side, fills to CLOSER TO ZERO
//    - Both positive: min(Blue, hist) - never beyond Blue line
//    - Both negative: max(Blue, hist) - never beyond Blue line
//
// EXAMPLES:
//
// Case 1: Blue = 0.8, hist = 0.5 (both positive)
//   Base: 0 to 0.8 (furthest)
//   GREEN: 0 to 0.5 (closer to zero, never beyond Blue at 0.8)
//   Visual: GREEN 0→0.5, RED/YELLOW 0→0.8
//
// Case 2: Blue = -0.8, hist = -0.5 (both negative)
//   Base: 0 to -0.8 (furthest)
//   GREEN: 0 to -0.5 (closer to zero, never beyond Blue at -0.8)
//   Visual: GREEN 0→-0.5, RED/YELLOW 0→-0.8
//
// Case 3: Blue = 0.5, hist = -0.3 (opposite sides)
//   Base: 0 to 0.5 AND 0 to -0.3 (fills both sides)
//   GREEN: None (opposite sides)
//   Visual: RED/YELLOW from -0.3 through 0 to 0.5
//
//+------------------------------------------------------------------+

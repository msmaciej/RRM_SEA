                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      if(green_condition)
      {
         // GREEN CONDITION: Create ribbon effect
         // GREEN histogram: From 0-line to histogram value
         g_HistGreen[i] = hist;
         
         // RED/YELLOW histogram: From 0-line to Blue line (overlaps green, creating ribbon)
         if(use_yellow)
         {
      ...
      {
         // NO GREEN: Normal histogram logic
         g_HistGreen[i] = EMPTY_VALUE;
         
         // RED/YELLOW histogram: From 0-line to histogram value (normal)
         if(use_yellow)
         {
            g_HistPos[i] = hist;
            g_HistNeg[i] = EMPTY_VALUE;

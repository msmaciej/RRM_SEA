...
//+------------------------------------------------------------------+
//|                         DPI_v30_FINAL.mq5                         |
//|   Purpose: MT4 DPI replica with CCI reset logic + GREEN ribbon   |
//|   Version: 30 FINAL - Complete ribbon with no gaps               |
//|                                                                  |
//|   COMPONENTS:                                                    |
...
//|   When both Blue line and histogram on SAME side of zero:       |
//|     - GREEN histogram: 0 to hist value                          |
//|     - RED/YELLOW ribbon: 0 to Blue line (overlays GREEN)        |
//|   When Blue line and histogram on OPPOSITE sides:               |
//|     - NO GREEN histogram                                         |
//|     - RED/YELLOW ribbon: 0 to Red contour (hist value)          |
//|                                                                  |
//|   This creates a continuous ribbon with NO GAPS                  |
//|                                                                  |
//|   CCI CALCULATION:                                               |
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      if(green_condition)
      {
         ...
         // This creates the ribbon effect with no gaps
         
         // GREEN histogram: From 0-line to histogram value
         g_HistGreen[i] = hist;
         
         // RED/YELLOW histogram: Extends from 0-line to Blue line
         // This overlays GREEN, creating continuous ribbon to Blue line
         if(use_yellow)
         {
      ...
      {
         // NO GREEN CONDITION: Blue line and histogram on opposite sides
         // No GREEN histogram needed
         g_HistGreen[i] = EMPTY_VALUE;
         
         // RED/YELLOW histogram: Extends from 0-line to Red contour (hist value)
         // This eliminates gap to Red contour line
         if(use_yellow)
         {
            g_HistPos[i] = hist;  // Extend to Red contour (NOT Blue line!)
            g_HistNeg[i] = EMPTY_VALUE;
         }
         else
         {
            g_HistPos[i] = EMPTY_VALUE;
            g_HistNeg[i] = hist;  // Extend to Red contour (NOT Blue line!)
         }
      }
   }
...
// USAGE NOTES:
//
// 1. RIBBON LOGIC - NO GAPS:
//    When GREEN condition (both same side of zero):
//      - GREEN: 0 → hist (histogram value)
//      - RED/YELLOW: 0 → Blue line (overlays GREEN)
//      - Result: Continuous ribbon from 0 to Blue line
//    
//    When NO GREEN (opposite sides):
//      - No GREEN histogram
//      - RED/YELLOW: 0 → hist (Red contour)
//      - Result: Continuous ribbon from 0 to Red contour
//
// 2. CCI RESET LOGIC:
...
//    - GREEN visible: Strong momentum alignment (both lines same side)
//
// 4. EA INTEGRATION WITH CCI:
//    More sophisticated filtering:
//    - Market SELL + Hist RED + No reset + No GREEN → Strong confirm
//    - Market SELL + Hist RED + Reset → Weak confirm
//    - Market SELL + Hist YELLOW → Avoid
//    - Market BUY + Hist YELLOW + No reset + No GREEN → Strong confirm
//    - Market BUY + Hist YELLOW + Reset → Weak confirm
//    - Market BUY + Hist RED → Avoid
//    - GREEN visible → Extra confirmation of momentum
//
// 5. CCI PARAMETER OPTIMIZATION:
...
//
// 6. GREEN HISTOGRAM INTERPRETATION:
//    - GREEN visible = Blue line and histogram on same side = momentum aligned
//    - No GREEN = Lines on opposite sides = momentum diverging
//    - GREEN provides visual confirmation of trend strength
//
// 7. TOGGLING FEATURES:
//    - Set InpEnableCCI = false to disable CCI resets

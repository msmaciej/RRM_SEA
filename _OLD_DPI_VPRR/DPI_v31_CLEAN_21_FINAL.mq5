...
//+------------------------------------------------------------------+
//|                      DPI_v31_FINAL.mq5                            |
//|   Purpose: Complete MT4 DPI replica - GREEN VISIBLE FIX          |
//|   Version: 31 FINAL - GREEN overlay width reduced for visibility |
//|                                                                  |
//|   FINAL FIX:                                                     |
//|   GREEN histogram uses narrower width (2 instead of 4) so it's   |
//|   visible as an overlay on top of the base Red/Yellow layer.     |
//|   Alternatively, we could use a different color or style.        |
//|                                                                  |
...
// Plot 3: Red histogram POSITIVE side (base layer)
...
// Plot 4: Red histogram NEGATIVE side (base layer)
...
// Plot 5: Yellow histogram POSITIVE side (base layer)
...
// Plot 6: Yellow histogram NEGATIVE side (base layer)
...
// Plot 7: GREEN histogram (narrower overlay, drawn LAST)
...
int OnInit()
{
   ...
   // GREEN overlay drawn LAST (on top, narrower width)
   ...
   // Apply user-selected colors to histogram plots
   PlotIndexSetInteger(3, PLOT_LINE_COLOR, InpColorBearish);  // Red Pos
   PlotIndexSetInteger(4, PLOT_LINE_COLOR, InpColorBearish);  // Red Neg
   PlotIndexSetInteger(5, PLOT_LINE_COLOR, InpColorBullish);  // Yellow Pos
   PlotIndexSetInteger(6, PLOT_LINE_COLOR, InpColorBullish);  // Yellow Neg
   PlotIndexSetInteger(7, PLOT_LINE_COLOR, InpColorGreen);    // GREEN overlay
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // CCI color logic - histogram determines the color for the entire ribbon
      ...
      // Draw base layer histograms
      // Same color (based on hist) applies to both positive and negative sides
      
      if(opposite_sides)
      {
         // OPPOSITE SIDES: Both filled with same color
         ...
         // Negative side - same color as positive
      ...
      {
         // SAME SIDE: Only one side filled
         if(positive_extent > 0.0)
         {
            // Positive side only
            if(hist_wants_yellow)
...
//
// ✅ Continuous ribbon (red contour to Blue line, no gaps)
// ✅ CCI color resets (Yellow/Red based on histogram + CCI)
// ✅ GREEN overlay VISIBLE (width=2, overlays on width=4 base)
// ✅ GREEN never beyond Blue line
//
// The GREEN histogram is drawn with width=2 while the base
// Red/Yellow histograms use width=4. This creates a visible
// green line down the center of the base histogram when both
// Blue and hist are on the same side of zero.
//
//+------------------------------------------------------------------+

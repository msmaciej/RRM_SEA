...
//+------------------------------------------------------------------+
//|                      DPI_v31_SOLUTION.mq5                         |
//|   Purpose: Complete MT4 DPI replica - ALL FEATURES WORKING       |
//|   Version: 31 SOLUTION - Final breakthrough                      |
//|                                                                  |
//|   BREAKTHROUGH SOLUTION:                                         |
//|   Uses FOUR histogram buffers instead of two:                    |
//|     - YellowPos / YellowNeg                                      |
//|     - RedPos / RedNeg                                            |
//|                                                                  |
//|   This allows:                                                   |
//|   ✅ Continuous ribbon (no gaps)                                 |
//|   ✅ CCI color resets working                                    |
//|   ✅ GREEN overlay correct                                       |
//|   ✅ Both sides filled when opposite                             |
//|                                                                  |
//|   LOGIC:                                                         |
//|   When Blue and hist on OPPOSITE sides:                          |
//|     - Positive side gets one color (Yellow or Red based on CCI)  |
//|     - Negative side gets another color (Yellow or Red based on CCI) |
//|   When on SAME side:                                             |
//|     - Base layer to furthest extent                              |
//|     - GREEN overlay to closer extent                             |
//+------------------------------------------------------------------+
#property strict
...
// Plot 3: Yellow histogram POSITIVE side
#property indicator_label4    "Hist_Yellow_Pos"
#property indicator_type4     DRAW_HISTOGRAM
#property indicator_color4    clrYellow
#property indicator_style4    STYLE_SOLID
#property indicator_width4    4

// Plot 4: Yellow histogram NEGATIVE side
#property indicator_label5    "Hist_Yellow_Neg"
#property indicator_type5     DRAW_HISTOGRAM
#property indicator_color5    clrYellow
#property indicator_style5    STYLE_SOLID
#property indicator_width5    4

// Plot 5: Red histogram POSITIVE side
#property indicator_label6    "Hist_Red_Pos"
#property indicator_type6     DRAW_HISTOGRAM
#property indicator_color6    clrRed
#property indicator_style6    STYLE_SOLID
#property indicator_width6    4

// Plot 6: Red histogram NEGATIVE side
#property indicator_label7    "Hist_Red_Neg"
#property indicator_type7     DRAW_HISTOGRAM
#property indicator_color7    clrRed
#property indicator_style7    STYLE_SOLID
#property indicator_width7    4

// Plot 7: GREEN histogram (drawn LAST so it overlays)
...
int OnInit()
{
   ...
   SetIndexBuffer(3, g_HistYellowPos, INDICATOR_DATA);
   SetIndexBuffer(4, g_HistYellowNeg, INDICATOR_DATA);
   SetIndexBuffer(5, g_HistRedPos,    INDICATOR_DATA);
   SetIndexBuffer(6, g_HistRedNeg,    INDICATOR_DATA);
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // Calculate CCI color for histogram
      // Logic: Yellow is "bullish", Red is "bearish"
      // When hist > 0: default Yellow, CCI can override to Red
      // When hist < 0: default Red, CCI can override to Yellow
      bool use_yellow_on_positive_side;
      bool use_yellow_on_negative_side;
      
      if(hist >= 0.0)
      {
         // Histogram is positive: default YELLOW, CCI<0 forces RED
         use_yellow_on_positive_side = !(InpEnableCCI && g_CCI[i] < 0.0);
         use_yellow_on_negative_side = false; // Not relevant when hist is positive
      }
      else
      {
         // Histogram is negative: default RED, CCI>=0 forces YELLOW
         use_yellow_on_negative_side = (InpEnableCCI && g_CCI[i] >= 0.0);
         use_yellow_on_positive_side = false; // Not relevant when hist is negative
      }
      
      // When Blue and hist are on opposite sides, we need to determine
      // color for EACH side independently based on which element is there
      bool blue_positive = (g_BlueCore[i] > 0.0);
      bool hist_positive = (hist > 0.0);
      bool opposite_sides = (blue_positive != hist_positive);
      
      // Calculate extents for each side
      double positive_extent = 0.0;
      double negative_extent = 0.0;
      
      if(opposite_sides)
      {
         // On opposite sides: fill both sides
         if(blue_positive)
         {
      ...
      {
         // On same side: fill to furthest extent
      ...
      // GREEN overlay calculation (never beyond Blue line)
      if(green_condition)
      {
         if(both_above_zero)
         {
            // Both positive: GREEN to closer to zero (minimum)
            g_HistGreen[i] = MathMin(g_BlueCore[i], hist);
         }
         else if(both_below_zero)
         {
            // Both negative: GREEN to closer to zero (maximum)
      ...
      // Now draw the base layer histograms with FOUR buffers
      // This allows independent color control on each side
      
      if(opposite_sides)
      {
         // OPPOSITE SIDES: Draw both positive and negative
         // Positive side
         if(positive_extent > 0.0)
         {
            if(use_yellow_on_positive_side)
            {
         ...
         {
            if(use_yellow_on_negative_side)
            {
      ...
      {
         // SAME SIDE: Only one side is filled
         if(positive_extent > 0.0)
         {
            // Positive side only
            if(use_yellow_on_positive_side)
...
//+------------------------------------------------------------------+
//
// BREAKTHROUGH SOLUTION EXPLANATION:
//
// The key insight is that MT5's DRAW_HISTOGRAM limitation requires
// separate buffers for each color AND each side when you need:
// 1. Independent color control (CCI resets)
// 2. Simultaneous positive and negative bars (opposite sides)
//
// FOUR BUFFERS SOLVE THIS:
// - YellowPos: Yellow bars on positive side
// - YellowNeg: Yellow bars on negative side
// - RedPos: Red bars on positive side
// - RedNeg: Red bars on negative side
//
// EXAMPLES:
//
// Case 1: Blue = 0.8, hist = 0.5 (same side, both positive)
//   CCI > 0: YellowPos = 0.8, all others EMPTY
//            GREEN = 0.5
//   CCI < 0: RedPos = 0.8, all others EMPTY
//            GREEN = 0.5
//
// Case 2: Blue = 0.5, hist = -0.3 (opposite sides)
//   CCI > 0: YellowPos = 0.5, YellowNeg = -0.3
//   CCI < 0: RedPos = 0.5, RedNeg = -0.3
//   No GREEN (opposite sides)
//
// Case 3: Blue = -0.8, hist = -0.5 (same side, both negative)
//   hist < 0 && CCI >= 0: YellowNeg = -0.8
//                         GREEN = -0.5
//   hist < 0 && CCI < 0:  RedNeg = -0.8
//                         GREEN = -0.5
//
// This achieves what MT4 could do natively but MT5 cannot with
// just two histogram buffers.
//
//+------------------------------------------------------------------+

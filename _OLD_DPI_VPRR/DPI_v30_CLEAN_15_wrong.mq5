...
//+------------------------------------------------------------------+
//|                      DPI_v30_ULTIMATE.mq5                         |
//|   Purpose: MT4 DPI replica - ULTIMATE VERSION                    |
//|   Version: 30 ULTIMATE - Perfect balance of all features         |
//|                                                                  |
//|   Combines:                                                      |
//|   - v08 approach: Always extend to Blue line (minimal gaps)     |
//|   - v14 GREEN fix: GREEN never extends beyond Blue line         |
//|   - Proper CCI reset logic maintained                            |
//|                                                                  |
//|   RIBBON STRATEGY:                                               |
//|   - GREEN condition: GREEN from 0 to min(Blue,hist)             |
//|                      RED/YELLOW from 0 to Blue                   |
//|   - NO GREEN: RED/YELLOW from 0 to Blue                         |
//|   - CCI resets: Always respected via use_yellow logic           |
//|                                                                  |
//|   Result: Minimal gaps, correct colors, GREEN never too far     |
//+------------------------------------------------------------------+
#property strict
                ...
                const int &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; --i)
   {
      ...
      // This is CRITICAL for CCI resets
      bool use_yellow;
      if(hist >= 0.0)
      {
      ...
      // ULTIMATE RIBBON LOGIC
      // Combines v08 approach (always extend to Blue) with v14 GREEN fix
      
      if(green_condition)
      {
         // GREEN CONDITION: Both same side of zero
         // GREEN histogram: From 0 to CLOSER TO ZERO (never beyond Blue line)
         if(both_above_zero)
         {
            // Both positive: GREEN to smaller (closer to zero)
            g_HistGreen[i] = MathMin(g_BlueCore[i], hist);
         }
         else // both_below_zero
         {
            // Both negative: GREEN to larger (closer to zero)
            g_HistGreen[i] = MathMax(g_BlueCore[i], hist);
         }
         
         // RED/YELLOW base: Always extends to Blue line (v08 approach)
         // This creates the ribbon effect with GREEN overlay
         if(use_yellow)
         {
            g_HistPos[i] = g_BlueCore[i];
            g_HistNeg[i] = EMPTY_VALUE;
         }
         else
         {
            g_HistPos[i] = EMPTY_VALUE;
            g_HistNeg[i] = g_BlueCore[i];
         }
      }
      else
      {
         ...
         // RED/YELLOW: Extends to Blue line (v08 approach)
         // This minimizes gaps to Blue line
         // CCI resets still work because use_yellow determines the color
         if(use_yellow)
         {
            g_HistPos[i] = g_BlueCore[i];
            g_HistNeg[i] = EMPTY_VALUE;
...
//+------------------------------------------------------------------+
//
// ULTIMATE VERSION - BEST BALANCE
//
// This version combines:
// 1. v08 approach: Always extend RED/YELLOW to Blue line
//    - Minimizes gaps to Blue line
//    - CCI resets work correctly (use_yellow logic maintained)
//
// 2. v14 GREEN fix: GREEN never extends beyond Blue line
//    - Both positive: GREEN = min(Blue, hist)
//    - Both negative: GREEN = max(Blue, hist)
//
// Trade-offs accepted:
// - When on opposite sides: Small gap to Red contour line (acceptable)
// - Gap shows divergence, which is informative
//
// What works perfectly:
// - CCI resets: ✓ Visible and correct
// - GREEN overlay: ✓ Never beyond Blue line
// - Ribbon continuity: ✓ Minimal gaps to Blue line
//
//+------------------------------------------------------------------+

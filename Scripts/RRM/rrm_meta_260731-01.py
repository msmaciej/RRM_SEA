...
def main():
    ...
    model_name = f"MetaModel_{preset}.csv"
    lines = [f"INTERCEPT,{clf.intercept_[0]:.10f}\n", f"THRESHOLD,{best_thr:.4f}\n"]
    for name, m, s, w in zip(feats, sc.mean_, sc.scale_, clf.coef_[0]):
        lines.append(f"FEATURE,{name},{m:.10f},{s:.10f},{w:.10f}\n")
    body = "".join(lines)

    targets = model_target_dirs(ev_path)
    print("wrote model to:")
    for d in targets:
        path = os.path.join(d, model_name)
        with open(path, "w") as f:
            f.write(body)
        tag = "  <- terminal MQL5\\Files (EA reads this at GATE startup)" \
              if os.sep + "Tester" + os.sep not in path else "  (agent sandbox)"
        print(f"  {path}{tag}")

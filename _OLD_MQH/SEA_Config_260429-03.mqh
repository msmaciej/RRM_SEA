...
struct ST_Settings
{
   ...
   // --- RESERVED: FastLine nested TSI periods (inactive — see DPI_Indicator.mq5 reserved block) ---
   int    DPI_TSI_FastR;           // Nested TSI first EMA period (default 5) — NOT used by active voter
   int    DPI_TSI_FastS;           // Nested TSI second EMA period (default 3) — NOT used by active voter
   int    VRC_ATR_Period;
   int    VRC_Lookback;
...
input int            Inp_RRM_ORG_TSI_FastR = 5;  // RRM_ORG: DPI Nested FastR (reserved, not active)
input int            Inp_RRM_ORG_TSI_FastS = 3;  // RRM_ORG: DPI Nested FastS (reserved, not active)
...
void InitializeConfig()
{
   ...
   Settings.DPI_TSI_FastR         = MathMax(1, Inp_RRM_ORG_TSI_FastR);  // Reserved — not used by active voter
   Settings.DPI_TSI_FastS         = MathMax(1, Inp_RRM_ORG_TSI_FastS);  // Reserved — not used by active voter

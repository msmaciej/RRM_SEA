...
void ApplyPreset(const EStrategyPreset preset, ST_Settings &cfg)
{
   ...
   if(preset == PRESET_TOPINVESTOR)
   {
      ...
      // Thresholds must be instrument-aware: Gold/indices have much wider pip ranges
      cfg.EmaFanFilterEnabled       = true;
      bool is_xau = (StringFind(_Symbol, "XAU") >= 0 || StringFind(_Symbol, "GOLD") >= 0);
      bool is_idx = (StringFind(_Symbol, "NAS") >= 0 || StringFind(_Symbol, "US30") >= 0 ||
                     StringFind(_Symbol, "SPX") >= 0 || StringFind(_Symbol, "GER") >= 0 ||
                     StringFind(_Symbol, "DAX") >= 0 || StringFind(_Symbol, "NDX") >= 0 ||
                     StringFind(_Symbol, "JP225") >= 0);
      double fan_mult = is_xau ? 20.0 : is_idx ? 15.0 : 1.0;

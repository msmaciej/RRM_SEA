...
input double      Inp_RM_RiskPercentDefault        = 2.0;         // RM: Risk: Default when Adaptive Risk Enable: false (was Inp_RiskPercent)
...
input bool        Inp_RM_UseAdaptiveRisk           = true;        // RM: Enable TF-based risk scaling (M1=1%, M5=1.5%, M15+=2%) (was Inp_UseAdaptiveRisk)
input bool        Inp_RM_UseMarginAdjustment       = true;        // RM: Enable instrument-aware adjustment
input double      Inp_RM_AdaptiveRisk_M1           = 1.0;         // RM: M1 timeframe (%) (was Inp_AdaptiveRisk_M1)
input double      Inp_RM_AdaptiveRisk_M5           = 1.5;         // RM: M5 timeframe (%) (was Inp_AdaptiveRisk_M5)
input double      Inp_RM_AdaptiveRisk_M15Plus      = 2.0;         // RM: M15+ timeframes (%) (was Inp_AdaptiveRisk_M15Plus)

// ── Optional Cushion Overrides (0 = use auto TF/JPY calculation) ──
input double      Inp_RM_Override_SL_Cushion       = 0.0;         // RM: SL cushion pips (0=auto; applies to Swing/PSAR modes)
input double      Inp_RM_Override_Trail_Cushion    = 0.0;         // RM: Trail cushion pips (0=auto; applies to PSAR trail)
input double      Inp_RM_Override_BE_Cushion       = 0.0;         // RM: BE cushion pips (0=auto; applies to breakeven buffer)

// ── Margin Level Instrument Adjustment ────────────────────────────
input double      Inp_RM_MarginAdj_Gold            = 0.8;         // RM: Gold/Metals (0.8 = use 80% of base margin level)
input double      Inp_RM_MarginAdj_Crypto          = 0.7;         // RM: Crypto (0.7 = use 70% of base margin level)
input double      Inp_RM_MarginAdj_Exotic          = 0.85;        // RM: Exotic pairs (0.85 = use 85% of base margin level)
input double      Inp_RM_MarginAdj_JPY             = 0.9;         // RM: JPY pairs (0.9 = use 90% of base margin level)
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_VETO_TE_RecheckBarClose      = false;      // TE: Re-check price drift vs Close[1]
input double      Inp_VETO_TE_BC_TolerancePips     = 3.0;        // TE: Drift tolerance in pips (if RecheckBarClose=true)
input int         Inp_VETO_TE_OpenDelaySeconds     = 0;          // TE: Delay after bar open in seconds (0=off)
input int         Inp_VETO_TE_SpreadMedianTicks    = 0;          // TE: Spread median filter tick window (0=off)
input group "╔════════════════════════════════════════════════════════╗";
...
// ╚════════════════════════════════════════════════════════════════╝
input bool        Inp_Filter_UseHTF                = false;       // MTF: DEPRECATED: Use Inp_Ind_MTF_Enabled
input ENUM_TIMEFRAMES Inp_Filter_HtfPeriod         = PERIOD_H4;   // MTF: DEPRECATED: Use Inp_MTF_TF1
input int         Inp_Filter_HtfEmaPeriod          = 89;          // MTF: DEPRECATED: Use Inp_MTF_EMA_Fast/Slow
...
input int        Inp_RRM_MinBarsAfterClose         = 3;              // RRM SL: RRM post-trade cooldown bars (0=off; was hardcoded 3)
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 RRM: (TS) Trailing Stop";
input group "╚════════════════════════════════════════════════════════╝";
input ETrailingMode Inp_RRM_TrailMode              = TRAIL_PSAR;     // RRM TS: RRM Trailing stop mode
...
input bool        Inp_RRM_FreezeTrailOnFlip        = true;           // RRM TS: Freeze trail on PSAR flip
input int         Inp_RRM_TrailPsarShiftDelay      = 2;              // RRM TS: PSAR DOT trailing shift (1..3) — bars back the SAR dot is read (TRAILING only)
...
input group "╚════════════════════════════════════════════════════════╝";
input ETPMode     Inp_RRM_ORG_TPMode               = TP_MODE_RR;     // RRM ORG TP: Take profit mode
input double      Inp_RRM_ORG_RRRatio              = 1.25;            // RRM ORG TP: RR ratio for TP_MODE_RR
//
...
input group "╚════════════════════════════════════════════════════════╝";
input ESLMode     Inp_RRM_ORG_SLMode               = SL_MODE_SWING;  // RRM ORG SL: Initial stop loss mode
input int         Inp_RRM_ORG_SwingLookback        = 34;             // RRM ORG SL: Swing lookback bars
input int         Inp_RRM_ORG_MinBarsAfterClose    = 3;              // RRM ORG SL: post-trade cooldown bars (0=off; was hardcoded 3)
//
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_RRM_ORG_BarClose_Require_Progressive_Momentum = false; // RRM ORG BC: BarClose Momentum
input int         Inp_RRM_ORG_BarClose_LookbackBars                 = 3;     // RRM ORG BC: BarClose Lookback (1-4 bars)
input group " ";
input group "╔════════════════════════════════════════════════════════╗";
...
input group "╚════════════════════════════════════════════════════════╝";
input double      Inp_RRM_ORG_BarClose_PipTolerance   = 0.0;         // RRM ORG Recov: BarClose: Allow close within N pips of target EMA (0=strict close>EMA)
input group " ";
input group "╔════════════════════════════════════════════════════════╗";
...
input int         Inp_RRM_ORG_HtfEmaPeriod         = 89;             // RRM ORG QA: HTF EMA period (0=use Inp_Filter_HtfEmaPeriod)
input int         Inp_RRM_ORG_Ema1Period           = 5;              // RRM ORG QA: EMA1 Period (was borrowing Inp_RRM_Ema1Period)
input int         Inp_RRM_ORG_Ema2Period           = 13;             // RRM ORG QA: EMA2 Period (was borrowing Inp_RRM_Ema2Period)
input int         Inp_RRM_ORG_Ema3Period           = 34;             // RRM ORG QA: EMA3 Period (was borrowing Inp_RRM_Ema3Period)
input int         Inp_RRM_ORG_Ema4Period           = 89;             // RRM ORG QA: EMA4 Period (was borrowing Inp_RRM_Ema4Period)
input int         Inp_RRM_ORG_CandleBody_Weight    = 1;              // RRM ORG QA: CandleBody vote weight (was borrowing Inp_CUSTOM_*)
input int         Inp_RRM_ORG_SmaConverge_Weight   = 1;              // RRM ORG QA: SmaConverge vote weight (was borrowing Inp_CUSTOM_*)
input int         Inp_RRM_ORG_TrailPsarShiftDelay  = 2;              // RRM ORG QA: PSAR DOT trail shift 1..3 (TRAILING only; was borrowing Inp_RRM_*)
input group " ";
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_RRM_ORG_LayerPBEnabled       = true;           // RRM ORG PB: Enable pullback-recovery state machine
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_RRM_ORG_EmaFanFilter         = true;           // RRM ORG Fan: EMA Fan Filter
...
input int         Inp_RRM_ORG_DDMaxConsecLosses    = 4;              // RRM ORG DD: Override max consecutive losses (0=use Inp_RRM_*)
...
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_Ema1                  = 9;      // TI EMA: EMA1: trailing exit reference
input int         Inp_TI_Ema2                  = 50;     // TI EMA: EMA2: primary bounce level
input int         Inp_TI_Ema3                  = 89;     // TI EMA: EMA3: intermediate structure
input int         Inp_TI_Ema4                  = 200;    // TI EMA: EMA4: major trend anchor (bias slow)

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Signal Architecture       ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_BlockUnorderedPhase   = true;   // TI Arch: Block trades in unordered EMA phase
input bool        Inp_TI_RequireMinPhaseConfirm = true;  // TI Arch: Require phase confirmation bars
input bool        Inp_TI_Emerging_AllowStrong  = false;  // TI Arch: Allow strong-layer trades in EM phase
input bool        Inp_TI_CloseOnReverse        = false;  // TI Arch: Close position on bias reversal

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Layer (Pullback Detection)║";
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_LayerBaselineLookback = 34;     // TI Layer: Layer baseline lookback bars
input double      Inp_TI_LayerPullbackRatio    = 0.5;    // TI Layer: Pullback depth ratio (0=none, 1=full retrace)
input double      Inp_TI_LayerRecoveryRatio    = 0.3;    // TI Layer: Recovery ratio required after pullback
input double      Inp_TI_LayerFlatRatio        = 0.15;   // TI Layer: Flat/sideways tolerance ratio

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — HTF Confirmation          ║";
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_MTF_EMA_Fast          = 50;     // TI HTF: HTF EMA fast (institutional standard)
input int         Inp_TI_MTF_EMA_Slow          = 200;    // TI HTF: HTF EMA slow (institutional standard)

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Exit Strategy             ║";
input group "╚════════════════════════════════════════════════════════╝";
input ETrailingMode   Inp_TI_TrailMode         = TRAIL_EMA;          // TI Exit: Trail mode (EMA9 exit is TI standard)
input int             Inp_TI_TrailEMA_Shift    = 1;      // TI Exit: Trail EMA shift: 1=tight, 2=one-bar cushion
input ETrailTrigger   Inp_TI_TrailTrigger      = TRIGGER_BREAKEVEN;  // TI Exit: When to start trailing
input double          Inp_TI_TrailStepPips     = 5.0;    // TI Exit: Trail step size (pips)
input double          Inp_TI_TrailProfitPercent = 2.0;   // TI Exit: Trail lock profit % (PROFIT_PERCENT trigger)
input ESLMode         Inp_TI_SLMode            = SL_MODE_SWING;      // TI Exit: SL placement mode
input ETPMode         Inp_TI_TPMode            = TP_MODE_RR;         // TI Exit: TP mode
input double          Inp_TI_RRRatio           = 2.0;    // TI Exit: Risk:Reward ratio
input EBeMode         Inp_TI_BE_Mode           = BE_MODE_R_MULTIPLE; // TI Exit: Breakeven mode
input double          Inp_TI_BE_RMultiple      = 1.0;    // TI Exit: Breakeven trigger at N×R profit

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — EMA Fan Filter            ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_EmaFanFilterEnabled   = true;   // TI Fan: Block overextended EMA fan entries
input double      Inp_TI_EmaFanBase_M1M5       = 50.0;   // TI Fan: Fan pip threshold base: M1–M5
input double      Inp_TI_EmaFanBase_M6M30      = 80.0;   // TI Fan: Fan pip threshold base: M6–M30
input double      Inp_TI_EmaFanBase_H1         = 120.0;  // TI Fan: Fan pip threshold base: H1
input double      Inp_TI_EmaFanBase_H2H4       = 200.0;  // TI Fan: Fan pip threshold base: H2–H4
input double      Inp_TI_EmaFanBase_H4Plus     = 350.0;  // TI Fan: Fan pip threshold base: H4+

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Drawdown Protection       ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_EnableDDProtection    = true;   // TI DD: Enable drawdown protection
input int         Inp_TI_MaxConsecutiveLosses  = 4;      // TI DD: Max consecutive losses before pause
input int         Inp_TI_MaxTradesPerDay       = 0;      // TI DD: Max trades per day (0=unlimited)
input double      Inp_TI_MaxDailyDrawdownPct   = 2.0;    // TI DD: Max daily drawdown % before pause

// ════════════════════════════════════════════════════════════════
// TOPINVESTOR: Quick Profile + core toggles
// ════════════════════════════════════════════════════════════════
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: Quick Profile                        ║";
input group "╚════════════════════════════════════════════════════════╝";
input ETIProfile  Inp_TI_Profile               = TI_MODERATE; // TI: Profile: Conservative(4v)|Moderate(7v)|Full(10v)
input int         Inp_TI_MinBarsAfterClose     = 3;      // TI: Post-trade cooldown bars (0=off)
input bool        Inp_TI_PhaseAllowEM          = true;   // TI: Allow Emerging phase (false=Trending-Market only)

// ════════════════════════════════════════════════════════════════
// TOPINVESTOR: Conservative voters — always active
// ════════════════════════════════════════════════════════════════
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: Conservative voters (always active)  ║";
input group "╚════════════════════════════════════════════════════════╝";
input double      Inp_TI_Psar_Step             = 0.02;   // TI Con: PSAR acceleration step
input double      Inp_TI_Psar_Max              = 0.2;    // TI Con: PSAR max acceleration
input int         Inp_TI_ADX_Period            = 14;     // TI Con: ADX period
input double      Inp_TI_ADX_Percentile        = 50.0;   // TI Con: ADX dynamic percentile threshold
input int         Inp_TI_ADX_Lookback          = 100;    // TI Con: ADX percentile lookback bars
input double      Inp_TI_ADX_Threshold_Accum   = 12.0;   // TI Con: ADX min in Accumulation phase
input double      Inp_TI_ADX_Threshold_Trend   = 25.0;   // TI Con: ADX min in Trending phase
input double      Inp_TI_ADX_Threshold_Dist    = 18.0;   // TI Con: ADX min in Distribution phase
input int         Inp_TI_CandleBody_AvgPeriod  = 10;     // TI Con: CandleBody average period (bars)
input double      Inp_TI_CandleBody_MaxMult    = 2.5;    // TI Con: CandleBody max spike multiplier

// ════════════════════════════════════════════════════════════════
// TOPINVESTOR: Moderate additions — active when Profile >= Moderate
// ════════════════════════════════════════════════════════════════
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: Moderate additions (Profile>=Moderate)║";
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_MACD_Fast             = 12;     // TI Mod: MACD fast EMA period
input int         Inp_TI_MACD_Slow             = 26;     // TI Mod: MACD slow EMA period
input int         Inp_TI_MACD_Signal           = 9;      // TI Mod: MACD signal period
input int         Inp_TI_MACD_FreshBars        = 5;      // TI Mod: MACD: max bars since histogram cross
input double      Inp_TI_MACD_SlopeMin         = 0.00001; // TI Mod: MACD: minimum histogram slope
input int         Inp_TI_CCI_Period            = 14;     // TI Mod: CCI period
input int         Inp_TI_BB_Period             = 20;     // TI Mod: Bollinger Bands period
input double      Inp_TI_BB_Deviation          = 2.0;    // TI Mod: Bollinger Bands deviation multiplier

// ════════════════════════════════════════════════════════════════
// TOPINVESTOR: Full additions — active when Profile >= Full
// ════════════════════════════════════════════════════════════════
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: Full additions (Profile>=Full)       ║";
input group "╚════════════════════════════════════════════════════════╝";
input double      Inp_TI_Fib_MinRetracement    = 0.38;   // TI Full: Fib min retracement depth (0.0–1.0)
input double      Inp_TI_Fib_MaxRetracement    = 0.618;  // TI Full: Fib max retracement depth (0.0–1.0)
input int         Inp_TI_Fib_SwingLookback     = 50;     // TI Full: Fib swing detection lookback bars
input double      Inp_TI_CandleBody_FullRatio  = 0.75;   // TI Full: Full: min close ratio (body quality gate)
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_CloseOnReverse        = false;          // Override: Entry Close On REverse
input EAutoStrategy  Inp_CUSTOM_AutoStrat          = STRAT_2EMA_POSITION;  // Override: Entry AutoStrat
input group "╔════════════════════════════════════════════════════════╗";
...
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Adx_Enabled       = false;          // Ind: TI Full: CUSTOM [ADX] Enable ADX
input EADXMode    Inp_CUSTOM_Ind_Adx_Mode          = ADX_MODE_STATIC; // Ind: TI Full: CUSTOM [ADX] Mode
input int         Inp_CUSTOM_Ind_Adx_Period        = 14;             // Ind: TI Full: CUSTOM [ADX] Period
input int         Inp_CUSTOM_Ind_Adx_Threshold     = 20;             // Ind: TI Full: CUSTOM [ADX] Threshold
input int         Inp_CUSTOM_Ind_Adx_Lookback      = 100;            // Ind: TI Full: CUSTOM [ADX] Lookback
input double      Inp_CUSTOM_Ind_Adx_Percentile    = 50.0;           // Ind: TI Full: CUSTOM [ADX] Percentile
input double      Inp_CUSTOM_Ind_Adx_Thr_Accum     = 12.0;           // Ind: TI Full: CUSTOM [ADX] Accumulation
input double      Inp_CUSTOM_Ind_Adx_Thr_Trending  = 25.0;           // Ind: TI Full: CUSTOM [ADX] Trending
input double      Inp_CUSTOM_Ind_Adx_Thr_Distrib   = 18.0;           // Ind: TI Full: CUSTOM [ADX] Distribution
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 ATR (Average True Range - Market Volatility - Non-directional)";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Atr_Enabled       = false;          // Ind: TI Full: CUSTOM [ATR] Enable ATR
input int         Inp_CUSTOM_Ind_Atr_Period        = 14;             // Ind: TI Full: CUSTOM [ATR] Period
input double      Inp_CUSTOM_Ind_Atr_VoteMinPips   = 5.0;            // Ind: TI Full: CUSTOM [ATR] Voting min pips
input double      Inp_CUSTOM_Ind_Atr_VoteMaxPips   = 50.0;           // Ind: TI Full: CUSTOM [ATR] Voting max pips
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 BB (Bollinger Bands - Market Volatility and Over bought/sold Levels)";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Bb_Enabled        = false;          // Ind: TI Full: CUSTOM [BB] Enable BB
input EBbMode     Inp_CUSTOM_Ind_Bb_Mode           = BB_TREND_FOLLOW; // Ind: TI Full: CUSTOM [BB] Mode
input int         Inp_CUSTOM_Ind_Bb_Period         = 20;             // Ind: TI Full: CUSTOM [BB] Period
input double      Inp_CUSTOM_Ind_Bb_Dev            = 2.0;            // Ind: TI Full: CUSTOM [BB] Deviation
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 CBody (Candle Body - Votes Against Overextended Candles (news/spikes))";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_CandleBody_Enabled    = false;      // Ind: TI Full: CUSTOM [CBody] Enable CB
input bool        Inp_CUSTOM_Ind_CandleBody_RequireDirection = true; // Ind: TI Full: CUSTOM [CBody] Require DIR
input int         Inp_CUSTOM_Ind_CandleBody_AvgPeriod  = 10;         // Ind: TI Full: CUSTOM [CBody] Average body period
input int         Inp_CUSTOM_Ind_CandleBody_CheckBars  = 1;          // Ind: TI Full: CUSTOM [CBody] Bars to check
input double      Inp_CUSTOM_Ind_CandleBody_MaxMult    = 3.0;        // Ind: TI Full: CUSTOM [CBody] Max body multiplier
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 CCI (Commodity Channel Index - Momentum Oscilator)";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Cci_Enabled       = false;          // Ind: TI Full: CUSTOM [CCI] Enable CCI
input ECciMode    Inp_CUSTOM_Ind_Cci_Mode          = CCI_TREND_ZERO; // Ind: TI Full: CUSTOM [CCI] Mode
input int         Inp_CUSTOM_Ind_Cci_Period        = 14;             // Ind: TI Full: CUSTOM [CCI] Period
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 CI (Choppiness Index - Block Trades in Ranging Market)";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_CI_Enabled        = false;          // Ind: TI Full: CUSTOM [CI] Enable ranging market filter
input int         Inp_CUSTOM_Ind_CI_Period         = 14;             // Ind: TI Full: CUSTOM [CI] Period
input double      Inp_CUSTOM_Ind_CI_RangingThreshold  = 61.8;        // Ind: TI Full: CUSTOM [CI] Threshold (>= this value = reject)
input group "╔════════════════════════════════════════════════════════╗";

...
input int             Inp_RRM_VPRR_RecoveryBars      = 5;             // RRM VPRR: Default recovery bars (1-10); per-instrument overrides below
input int             Inp_RRM_VPRR_Weight            = 1;             // RRM VPRR: Vote weight
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 RRM VPRR: Per-Instrument Tuning";
input group "╚════════════════════════════════════════════════════════╝";
input double          Inp_RRM_VPRR_MinRatio_Gold       = 1.0;           // RRM VPRR Gold (XAU): MinRatio — M5:0.8 M15:1.0 H1:0.95 H4:0.9 (auto-adjusted by TF mult)
input int             Inp_RRM_VPRR_RecBars_Gold         = 3;             // RRM VPRR Gold: RecoveryBars — M15:3 H1:2 H4:1
input double          Inp_RRM_VPRR_MinRatio_Silver      = 0.9;           // RRM VPRR Silver (XAG): MinRatio — thinner market, fewer institutions (0.85–1.0)
input int             Inp_RRM_VPRR_RecBars_Silver       = 2;             // RRM VPRR Silver: RecoveryBars — fewer bars (thinner market)
input double          Inp_RRM_VPRR_MinRatio_IndicesUS   = 1.1;           // RRM VPRR US Indices (NAS/US30/SPX): MinRatio — deep liquid futures (1.1–1.3)
input int             Inp_RRM_VPRR_RecBars_IndicesUS    = 3;             // RRM VPRR US Indices: RecoveryBars — NY session only (14:30–21:00 UTC)
input double          Inp_RRM_VPRR_MinRatio_IndicesEU   = 1.0;           // RRM VPRR EU Indices (DAX/FTSE/FRA40): MinRatio — thinner than US (1.0–1.2)
input int             Inp_RRM_VPRR_RecBars_IndicesEU    = 3;             // RRM VPRR EU Indices: RecoveryBars — Frankfurt/London session only
input double          Inp_RRM_VPRR_MinRatio_Oil         = 0.9;           // RRM VPRR Oil (WTI/Brent): MinRatio (0.9–1.0)
input int             Inp_RRM_VPRR_RecBars_Oil          = 3;             // RRM VPRR Oil: RecoveryBars
input double          Inp_RRM_VPRR_MinRatio_Crypto      = 0.7;           // RRM VPRR Crypto (BTC/ETH): MinRatio — retail-dominated, erratic spikes (0.7–0.8)
input int             Inp_RRM_VPRR_RecBars_Crypto       = 2;             // RRM VPRR Crypto: RecoveryBars
input double          Inp_RRM_VPRR_MinRatio_Equities    = 1.0;           // RRM VPRR Equities (NVDA/AAPL etc.): MinRatio — real exchange vol, earnings-aware (0.9–1.1)
input int             Inp_RRM_VPRR_RecBars_Equities     = 2;             // RRM VPRR Equities: RecoveryBars — fast institutional execution
input double          Inp_RRM_VPRR_MinRatio_FX          = 0.7;           // RRM VPRR FX: MinRatio — tick vol only, approximation (0.6–0.7)
input int             Inp_RRM_VPRR_RecBars_FX           = 3;             // RRM VPRR FX: RecoveryBars
input double          Inp_RRM_VPRR_MinRatio_NonFXTick   = 0.8;           // RRM VPRR non-FX tick fallback: MinRatio (broker missing real vol)
input group "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557";
input group "\u2551   \U0001f4ca RRM VPRR: Timeframe Adjustment (auto-applied to MinRatio + RecBars)";
input group "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d";
input double          Inp_RRM_VPRR_TF_Mult_M5           = 0.85;          // RRM VPRR TF: M5 MinRatio multiplier (noisier — loosen by 15%)
input double          Inp_RRM_VPRR_TF_Mult_M15          = 1.00;          // RRM VPRR TF: M15 MinRatio multiplier (baseline — no adjustment)
input double          Inp_RRM_VPRR_TF_Mult_H1           = 0.95;          // RRM VPRR TF: H1 MinRatio multiplier (fewer bars available — loosen slightly)
input double          Inp_RRM_VPRR_TF_Mult_H4Plus       = 0.90;          // RRM VPRR TF: H4+ MinRatio multiplier (very few cycles — loosen)
input bool            Inp_RRM_VPRR_TF_ReduceRecBars     = true;          // RRM VPRR TF: Reduce RecoveryBars by 1 on H4+ and M5 (prevents waiting too long for recovery)
input group "╔════════════════════════════════════════════════════════╗";
...
input group "╚════════════════════════════════════════════════════════╝";
input ETPMode     Inp_RRM_ORG_TPMode               = TP_MODE_RR;     // RRM ORG TP: mode
input double      Inp_RRM_ORG_RRRatio              = 1.5;            // RRM ORG TP: RR ratio for TP_MODE_RR
//
...
input int             Inp_RRM_ORG_VPRR_RecoveryBars = 5;              // RRM ORG VPRR: VPRR recovery bars — default for all instruments (1-10); per-instrument overrides below
input int             Inp_RRM_ORG_VPRR_Weight       = 1;              // RRM ORG VPRR: VPRR vote weight
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 RRM_ORG VPRR: Per-Instrument Tuning";
input group "╚════════════════════════════════════════════════════════╝";
// Gold/Silver: real exchange volume — strong institutional footprint on COMEX
// Session: London+NY overlap (13:00–17:00 UTC). Avoid Asian hours (00:00–07:00 UTC).
// MinRatio 1.0–1.2 on M15; 0.9–1.1 on H1. RecoveryBars 3 on M15, 2 on H1.
input double          Inp_RRM_ORG_VPRR_MinRatio_Gold       = 1.0;       // RRM ORG VPRR Gold (XAU): MinRatio — M15:1.0 H1:0.95 H4:0.9
input int             Inp_RRM_ORG_VPRR_RecBars_Gold         = 3;         // RRM ORG VPRR Gold: RecoveryBars — M15:3 H1:2 H4:1
input double          Inp_RRM_ORG_VPRR_MinRatio_Silver      = 0.9;       // RRM ORG VPRR Silver (XAG): MinRatio — thinner market (0.85–1.0)
input int             Inp_RRM_ORG_VPRR_RecBars_Silver       = 2;         // RRM ORG VPRR Silver: RecoveryBars
input double          Inp_RRM_ORG_VPRR_MinRatio_IndicesUS   = 1.1;       // RRM ORG VPRR US Indices (NAS/US30/SPX): MinRatio (1.1–1.3)
input int             Inp_RRM_ORG_VPRR_RecBars_IndicesUS    = 3;         // RRM ORG VPRR US Indices: RecoveryBars — NY session only
input double          Inp_RRM_ORG_VPRR_MinRatio_IndicesEU   = 1.0;       // RRM ORG VPRR EU Indices (DAX/FTSE/FRA40): MinRatio (1.0–1.2)
input int             Inp_RRM_ORG_VPRR_RecBars_IndicesEU    = 3;         // RRM ORG VPRR EU Indices: RecoveryBars — Frankfurt/London session only
input double          Inp_RRM_ORG_VPRR_MinRatio_Oil         = 0.9;       // RRM ORG VPRR Oil (WTI/Brent): MinRatio (0.9–1.0)
input int             Inp_RRM_ORG_VPRR_RecBars_Oil          = 3;         // RRM ORG VPRR Oil: RecoveryBars
input double          Inp_RRM_ORG_VPRR_MinRatio_Crypto      = 0.7;       // RRM ORG VPRR Crypto (BTC/ETH): MinRatio (0.7–0.8)
input int             Inp_RRM_ORG_VPRR_RecBars_Crypto       = 2;         // RRM ORG VPRR Crypto: RecoveryBars
input double          Inp_RRM_ORG_VPRR_MinRatio_Equities    = 1.0;       // RRM ORG VPRR Equities (NVDA/AAPL etc.): MinRatio (0.9–1.1)
input int             Inp_RRM_ORG_VPRR_RecBars_Equities     = 2;         // RRM ORG VPRR Equities: RecoveryBars
input double          Inp_RRM_ORG_VPRR_MinRatio_FX          = 0.7;       // RRM ORG VPRR FX: MinRatio — tick vol approximation (0.6–0.7)
input int             Inp_RRM_ORG_VPRR_RecBars_FX           = 3;         // RRM ORG VPRR FX: RecoveryBars
input double          Inp_RRM_ORG_VPRR_MinRatio_NonFXTick   = 0.8;       // RRM ORG VPRR non-FX tick fallback: MinRatio
input group "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557";
input group "\u2551   \U0001f4ca RRM_ORG VPRR: Timeframe Adjustment";
input group "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d";
input double          Inp_RRM_ORG_VPRR_TF_Mult_M5           = 0.85;      // RRM ORG VPRR TF: M5 MinRatio multiplier
input double          Inp_RRM_ORG_VPRR_TF_Mult_M15          = 1.00;      // RRM ORG VPRR TF: M15 MinRatio multiplier (baseline)
input double          Inp_RRM_ORG_VPRR_TF_Mult_H1           = 0.95;      // RRM ORG VPRR TF: H1 MinRatio multiplier
input double          Inp_RRM_ORG_VPRR_TF_Mult_H4Plus       = 0.90;      // RRM ORG VPRR TF: H4+ MinRatio multiplier
input bool            Inp_RRM_ORG_VPRR_TF_ReduceRecBars     = true;      // RRM ORG VPRR TF: Reduce RecoveryBars by 1 on H4+ and M5
input group "╔════════════════════════════════════════════════════════╗";
...
input int             Inp_TI_VPRR_RecoveryBars       = 5;             // TI VPRR: Default recovery bars (1-10); per-instrument overrides below
input int             Inp_TI_VPRR_Weight             = 1;             // TI VPRR: Vote weight
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 TI VPRR: Per-Instrument Tuning";
input group "╚════════════════════════════════════════════════════════╝";
input double          Inp_TI_VPRR_MinRatio_Gold         = 1.0;           // TI VPRR Gold (XAU): MinRatio — M15:1.0 H1:0.95 H4:0.9
input int             Inp_TI_VPRR_RecBars_Gold           = 3;             // TI VPRR Gold: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_Silver        = 0.9;           // TI VPRR Silver (XAG): MinRatio (0.85–1.0)
input int             Inp_TI_VPRR_RecBars_Silver         = 2;             // TI VPRR Silver: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_IndicesUS     = 1.1;           // TI VPRR US Indices (NAS/US30/SPX): MinRatio (1.1–1.3)
input int             Inp_TI_VPRR_RecBars_IndicesUS      = 3;             // TI VPRR US Indices: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_IndicesEU     = 1.0;           // TI VPRR EU Indices (DAX/FTSE/FRA40): MinRatio (1.0–1.2)
input int             Inp_TI_VPRR_RecBars_IndicesEU      = 3;             // TI VPRR EU Indices: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_Oil           = 0.9;           // TI VPRR Oil (WTI/Brent): MinRatio (0.9–1.0)
input int             Inp_TI_VPRR_RecBars_Oil            = 3;             // TI VPRR Oil: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_Crypto        = 0.7;           // TI VPRR Crypto (BTC/ETH): MinRatio (0.7–0.8)
input int             Inp_TI_VPRR_RecBars_Crypto         = 2;             // TI VPRR Crypto: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_Equities      = 1.0;           // TI VPRR Equities (NVDA/AAPL etc.): MinRatio (0.9–1.1)
input int             Inp_TI_VPRR_RecBars_Equities       = 2;             // TI VPRR Equities: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_FX            = 0.7;           // TI VPRR FX: MinRatio — tick vol (0.6–0.7)
input int             Inp_TI_VPRR_RecBars_FX             = 3;             // TI VPRR FX: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_NonFXTick     = 0.8;           // TI VPRR non-FX tick fallback: MinRatio
input group "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557";
input group "\u2551   \U0001f4ca TI VPRR: Timeframe Adjustment";
input group "\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d";
input double          Inp_TI_VPRR_TF_Mult_M5             = 0.85;          // TI VPRR TF: M5 MinRatio multiplier
input double          Inp_TI_VPRR_TF_Mult_M15            = 1.00;          // TI VPRR TF: M15 MinRatio multiplier (baseline)
input double          Inp_TI_VPRR_TF_Mult_H1             = 0.95;          // TI VPRR TF: H1 MinRatio multiplier
input double          Inp_TI_VPRR_TF_Mult_H4Plus         = 0.90;          // TI VPRR TF: H4+ MinRatio multiplier
input bool            Inp_TI_VPRR_TF_ReduceRecBars       = true;          // TI VPRR TF: Reduce RecoveryBars by 1 on H4+ and M5
...
input group "╔═══════════════════════════════════════════════════════╗";
input group "║   📊 STEP 5b: Fibonacci Retracement Voter              ║";
input group "╚═══════════════════════════════════════════════════════╝";

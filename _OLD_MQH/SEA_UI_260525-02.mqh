...
void SEA_UI_UpdateSettingsPanel(EMarketPhase current_phase = PHASE_UNORDERED)
{
   ...
   // Resolve _Period: PERIOD_CURRENT (0) must map to the actual chart TF.
   // EnumToString(PERIOD_CURRENT) returns "PERIOD_CURRENT" — not useful for display.
   ENUM_TIMEFRAMES _eff_period = (_Period == PERIOD_CURRENT) ? (ENUM_TIMEFRAMES)Period() : _Period;
   AddLine(StringFormat("SYMBOL:   %s %s", _Symbol, EnumToString(_eff_period)), v_clr, lines, line_clrs);

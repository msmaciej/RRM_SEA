...
void EvalWithEngine(int shift, CSignalEngine &eng, int bias)
{
   ...
   // ── I: DPI ────────────────────────────────────────────────────
   if(In_I_DPI)
      if(!eng.Scanner_Check_DPI(bias, s)) return;

   // ── I: PSAR ───────────────────────────────────────────────────
   if(In_I_PSAR_Flip)
   { if(!eng.Scanner_Check_PSAR_Flip(bias, s)) return; }
   else if(In_I_PSAR)
   { if(!eng.Scanner_Check_PSAR(bias, s)) return; }
...
{
   ZeroMemory(s);
   // Note: symbol is passed separately to g_eng.Init() — no field in ST_Settings

   // EMA periods — correct field names from ST_Settings struct
   s.P_Ema1              = In_EMA1;
   s.P_Ema2              = In_EMA2;
   s.P_Ema3              = In_EMA3;
   s.P_Ema4              = In_EMA4;
   s.MaType              = METHOD_EMA;   // EMaMethod enum, not ENUM_MA_METHOD
   s.ma_h_shift          = 0;
   s.ma_v_shift          = 0;

   // Bias — engine uses BIAS_4EMA for handle creation regardless of scanner mode
   s.BiasMode            = BIAS_4EMA;
   s.BiasEnabled         = true;

   // Phase — allow both TM and EM (bias filter handled in scanner directly)
   ...
   // DPI — correct field names
   s.Ind_Dpi_Enabled        = In_I_DPI;
   s.DPI_MACD_Fast          = In_DPI_Fast;
   s.DPI_MACD_Slow          = In_DPI_Slow;
   s.DPI_RedEMA_A           = In_DPI_Signal;   // signal/red EMA period
   s.DPI_UseCCIReset        = In_DPI_CCI;
   s.DPI_IgnoreCCIForVote   = !In_DPI_CCI;
   s.DPI_CCI_Period         = In_DPI_CCI_Period;

   // PSAR — correct field names
   s.Ind_Psar_Enabled       = (In_I_PSAR || In_I_PSAR_Flip);
   s.P_PsarStep             = In_PSAR_Step;
   s.P_PsarMax              = In_PSAR_Max;
   ...
   s.MTF_TF1                = In_MTF_TF;
   s.MTF_TF2                = PERIOD_CURRENT;   // single TF mode
   s.MTF_EMA_Fast           = In_MTF_EMA;
   s.MTF_EMA_Slow           = In_MTF_EMA;       // same = slope mode

...
void ScanBar(int shift)
{
   ...
   if(TS_LayerS || TS_LayerM || TS_LayerW)
   {
      ...
      // When bias is absent: expire any RECOVERED states (can't fire without bias)
      // but preserve DETECTED (pullback in progress — keep tracking).
      if(!doL) g_eng_long.Scanner_ExpireRecovered();
      if(!doS) g_eng_short.Scanner_ExpireRecovered();

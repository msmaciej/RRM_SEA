...
private:   
   ...
   bool Check_ADX(int shift) {
      ...
      // Update rolling history for dynamic modes (skip for static/rising — no history needed)
      if(m_settings.ADX_Mode != ADX_MODE_STATIC && m_settings.ADX_Mode != ADX_MODE_RISING)
      ...
      switch(m_settings.ADX_Mode) {
         ...
         {
            // Institutional flow mode: ADX must be above threshold AND rising.
            // Rising ADX = trend participation increasing (institutional flow building).
            // Falling ADX above threshold = distribution (institutions exiting into trend).
            threshold = (double)m_settings.T_Adx;
            double adx_prev = GetVal(h_adx, shift + 1);
            bool above_threshold = (adx >= threshold);
            bool is_rising = (adx > adx_prev && adx_prev > 0.0);
            // Override result directly — both conditions must hold
            m_cachedADXThreshold = threshold;
            m_ind_cache.adx_value = adx;
            bool rising_result = (above_threshold && is_rising);
            m_ind_cache.adx_result = rising_result ? 1 : 0;
            if(m_settings.DebugFlow) {
               if(m_settings.Ind_Adx_Enabled)
                  DebugLog(StringFormat("[IND_ADX] ENABLED | Value=%.2f (prev=%.2f) | Threshold=%.2f | Mode=RISING | Above=%s Rising=%s | Result: %s",
                                        adx, adx_prev, threshold,
                                        above_threshold ? "Y" : "N",
                                        is_rising ? "Y" : "N",
                                        rising_result ? "PASS" : "FAIL"));
               else
                  DebugLog("[IND_ADX] DISABLED - skipped");
            }
            return rising_result;
         }
      }
   ...
   // BUG FIX: Previously read m_diag_last_atr_pips which was set by
   // CheckFilters() — but CheckFilters() was never called, so ATR
   // always read 0.0 and the MinPips check always failed.
   // Now computes ATR directly from the indicator handle.
   bool Check_ATR(int bias, int shift)
   {
      if(IsCacheValidForShift(shift) && m_ind_cache.atr_result != -1)
         return (m_ind_cache.atr_result == 1);

      // Compute ATR pips directly from indicator handle
      double atr_raw = GetVal(h_atr, shift);
      double atr_pips = GlobalAtrPips(atr_raw, m_symbol);

      // Update diagnostic cache so UI/logs show the correct value
...
public:
   ...
   bool Init(ST_Settings &sets, string symbol) {
      ...
      // BUG FIX: VRC depends on h_atr for GetVolatilityRegime(), so create
      // the ATR handle when either ATR voter or VRC is enabled.

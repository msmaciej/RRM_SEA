...
//| STEP 6 — Display                                                 |
//+------------------------------------------------------------------+
input group "=== STEP 5: Display — same settings as EA UI panel ==="
input color           Color_Long              = clrDodgerBlue;  // LONG signal line color
input color           Color_Short             = clrRed;         // SHORT signal line color
input ENUM_LINE_STYLE LineStyle               = STYLE_DOT;      // Signal line style
input int             LineWidth               = 1;              // Signal line width
input int             BarsBack               = 500;             // Bars to scan back on startup
input group "--- Panel style (match these to EA: UI Font/Size/Spacing) ---"
input string          Inp_UI_PanelFont        = "Arial";        // Panel font (match EA setting)
input int             Inp_UI_PanelFontSize    = 10;             // Panel font size (match EA setting)
input int             Inp_UI_PanelLineSpacingPx = 14;           // Panel line spacing px (match EA setting)
input int             Inp_UI_PanelX           = 10;             // Panel X position (px from corner)
input int             Inp_UI_PanelY           = 20;             // Panel Y position (px from corner)
...
void DrawInfoPanel()
{
   ...
   int x = Inp_UI_PanelX;
   int y = Inp_UI_PanelY;
   int dy = (Inp_UI_PanelLineSpacingPx > 0) ? Inp_UI_PanelLineSpacingPx : (Inp_UI_PanelFontSize + 4);
   int fs = Inp_UI_PanelFontSize;
   string font = Inp_UI_PanelFont;
   ...
   #define PL(id,yoff,txt,clr) { \
      ...
      if(ObjectFind(0,_n)<0){ \
         ...
         ObjectSetInteger(0,_n,OBJPROP_CORNER,(int)Inp_UI_PanelCorner); \
   ...
   // ── Signal counts ───────────────────────────────────────────────
   PL(r, dy*r, "Signals  (last " + (string)BarsBack + " bars):", clrSilver)   r++;

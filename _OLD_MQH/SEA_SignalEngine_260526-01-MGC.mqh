...
private:   
   ...
   //+------------------------------------------------------------------+
   //| _ValidateVPRRExternalSymbol — OnInit() startup check (private)   |
   //+------------------------------------------------------------------+
   // Called via public wrapper ValidateVPRRExternalSymbol() from OnInit().
   bool _ValidateVPRRExternalSymbol()
   {
      string ext = m_settings.VPRR_ExternalSymbol;

      Print("╔══════════════════════════════════════════════════════════╗");
      Print("║  VPRR EXTERNAL SYMBOL VALIDATION                        ║");
      PrintFormat("║  Traded symbol  : %-38s ║", m_symbol);
      PrintFormat("║  Proxy symbol   : %-38s ║", (StringLen(ext) > 0 ? ext : "(empty)"));
      Print("╠══════════════════════════════════════════════════════════╣");

      // 1. Empty symbol — configuration error
      if(StringLen(ext) == 0)
      {
         Print("║  ❌ FAIL: Inp_VPRR_ExternalSymbol is empty.             ║");
         Print("║     Set it to e.g. \"GC\" or \"MGC\" in EA inputs.         ║");
         Print("║     VPRR will BLOCK all entries until corrected.        ║");
         Print("╚══════════════════════════════════════════════════════════╝");
         return false;
      }

      // 2. Symbol must be selectable (in Market Watch or server symbol list)
      if(!SymbolSelect(ext, true))
      {
         PrintFormat("║  ❌ FAIL: Symbol \"%s\" not found on this broker.  ║", ext);
         Print("║     Add it to Market Watch manually, then restart EA.   ║");
         Print("║     VPRR will BLOCK all entries until corrected.        ║");
         Print("╚══════════════════════════════════════════════════════════╝");
         return false;
      }

      // 3. Must have bars on the current timeframe
      int ext_bars = Bars(ext, PERIOD_CURRENT);
      if(ext_bars < 10)
      {
         PrintFormat("║  ⚠️  WARNING: \"%s\" has only %d bars on TF %s.    ║",
                     ext, ext_bars, EnumToString((ENUM_TIMEFRAMES)_Period));
         Print("║     Broker may not carry this TF yet. Monitor journal.  ║");
      }
      else
      {
         PrintFormat("║  ✅ Bars available : %d on TF %s               ║",
                     ext_bars, EnumToString((ENUM_TIMEFRAMES)_Period));
      }

      // 4. Probe real volume on the most recent closed bar
      datetime bar_time = iTime(m_symbol, PERIOD_CURRENT, 1);
      int ext_shift     = iBarShift(ext, PERIOD_CURRENT, bar_time, true);
      bool real_vol_ok  = false;
      ...
      if(ext_shift >= 0)
      {
         ...
         {
            real_vol_ok = true;
            PrintFormat("║  ✅ Real volume    : %I64d contracts (bar shift %d)   ║",
                        probe[0], ext_shift);
         }
         else
         {
            Print("║  ❌ FAIL: CopyRealVolume returned 0 or no data.         ║");
            PrintFormat("║     Broker does not provide real volume for \"%s\".  ║", ext);
            Print("║     VPRR will BLOCK all entries — wrong broker/symbol.  ║");
         }
      }
      else
      {
         PrintFormat("║  ❌ FAIL: iBarShift could not align \"%s\" to        ║", ext);
         PrintFormat("║     traded symbol bar at %s.                   ║",
                     TimeToString(bar_time, TIME_DATE|TIME_MINUTES));
         Print("║     Sessions may not overlap. Check CME trading hours.  ║");
      }

      // 5. Time-alignment sanity: warn if sessions differ significantly
      if(real_vol_ok)
      {
         datetime ext_bar_open = iTime(ext, PERIOD_CURRENT, ext_shift);
         long     diff_sec     = MathAbs((long)(bar_time - ext_bar_open));
         if(diff_sec > PeriodSeconds(PERIOD_CURRENT))
         {
            PrintFormat("║  ⚠️  Bar-time gap   : %d sec (> 1 bar). Check TF match. ║",
                        (int)diff_sec);
         }
         else
         {
            PrintFormat("║  ✅ Bar alignment  : %d sec gap — OK               ║",
                        (int)diff_sec);
         }
      }
      ...
      if(real_vol_ok)
      {
         PrintFormat("║  ✅ VPRR EXTERNAL ACTIVE: volume from \"%s\"         ║", ext);
         Print("║     COMEX real volume will gate all metal entries.      ║");
      }
      else
      {
         Print("║  ❌ VPRR EXTERNAL DISABLED: no real volume available.   ║");
         Print("║     All VPRR-gated entries will be BLOCKED.             ║");
         Print("║     Fix broker/symbol before trading live.              ║");
      }
      Print("╚══════════════════════════════════════════════════════════╝");
   ...
   //   AUTO/REAL : try CopyRealVolume on the traded symbol; tick fallback for AUTO.
   //   TICK      : broker tick count on the traded symbol.
   //   EXTERNAL  : CopyRealVolume from proxy symbol (e.g. "GC" for XAUUSD).
   //               Bar is aligned by open-time via iBarShift.
   //               Returns 0 (VPRR blocks) if symbol unavailable or no real vol.
   // Records which source was used in m_vprr_last_real for the UI label.
   long GetCurrentBarVolume(int shift)
   {
      // ── EXTERNAL: proxy symbol real volume ───────────────────────────
      if(m_settings.VPRR_VolumeType == VPRR_VOL_EXTERNAL)
      {
         ...
         if(StringLen(ext) > 0)
         {
            ...
            {
               long ext_vol[];
               if(CopyRealVolume(ext, PERIOD_CURRENT, ext_shift, 1, ext_vol) == 1
                  && ext_vol[0] > 0)
               {
         ...
         // Empty symbol, bar not found, or no real vol → block entry (return 0)
         m_vprr_last_real = false;
         return 0;
      }

      // ── AUTO / REAL: try CopyRealVolume on the traded symbol ─────────
         ...
         m_settings.VPRR_VolumeType == VPRR_VOL_AUTO)
      {
         ...
         // REAL forced but unavailable → block; AUTO continues to tick fallback.
         if(m_settings.VPRR_VolumeType == VPRR_VOL_REAL)
         {
            m_vprr_last_real = false;
            return 0;
...
public:
   // VPRR external symbol startup validation (called from OnInit)
   bool ValidateVPRRExternalSymbol() { return _ValidateVPRRExternalSymbol(); }

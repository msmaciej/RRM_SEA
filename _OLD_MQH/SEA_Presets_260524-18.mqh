...
void ApplyPreset(const EStrategyPreset preset, ST_Settings &cfg)
{
   ...
   if(preset == PRESET_RRM_ORG)
   {
      ...
      cfg.TrailEMA_Period           = (Inp_RRM_ORG_TrailEMA_Period > 0)
                                          ? Inp_RRM_ORG_TrailEMA_Period
                                          : (int)cfg.P_Ema1;
      cfg.TrailEMA_Shift            = MathMax(1, MathMin(3, Inp_RRM_ORG_TrailEMA_Shift));
      // EMA cushion: 0 = fall back to auto instrument-scaled PSAR_TrailPipsCushion

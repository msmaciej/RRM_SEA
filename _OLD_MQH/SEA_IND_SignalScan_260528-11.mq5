...
void Eval(int shift, CSignalEngine &eng, int bias)
{
   ...
   if(any_layer)
   {
      ...
      // Per-layer recovery expiry tracking (independent per engine direction)
      // rec_bar pointers reference the correct set based on bias
      int &rec_s = (bias==1) ? g_layer_s_rec_bar_long  : g_layer_s_rec_bar_short;
      int &rec_m = (bias==1) ? g_layer_m_rec_bar_long  : g_layer_m_rec_bar_short;
      int &rec_w = (bias==1) ? g_layer_w_rec_bar_long  : g_layer_w_rec_bar_short;

      // Track when each layer first entered RECOVERED; expire after PB_RecoveryBars
      if(eng.GetLayerSPullbackState()==LAYER_PB_RECOVERED) { if(rec_s<0) rec_s=shift; }
      else rec_s = -1;
      if(eng.GetLayerMPullbackState()==LAYER_PB_RECOVERED) { if(rec_m<0) rec_m=shift; }
      else rec_m = -1;
      if(eng.GetLayerWPullbackState()==LAYER_PB_RECOVERED) { if(rec_w<0) rec_w=shift; }
      else rec_w = -1;

      // Expire layers whose recovery window has elapsed (rec_bar-shift = bars elapsed)
      // Note: shift decreases as we move forward in time (oldest=high, newest=low)
      if(rec_s >= 0 && (rec_s - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(3); rec_s=-1; }
      if(rec_m >= 0 && (rec_m - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(2); rec_m=-1; }
      if(rec_w >= 0 && (rec_w - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(1); rec_w=-1; }
      ...
      if(fired_layer==3) rec_s=-1;
      else if(fired_layer==2) rec_m=-1;
      else rec_w=-1;
      return;
   }

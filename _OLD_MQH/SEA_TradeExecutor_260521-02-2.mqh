   ...
   void UpdatePositionExcursion(ulong ticket) {
      ...
      switch(m_settings.TrailMode) {
         ...
         case TRAIL_EMA:
         {
            ...
            // Guard: if TrailStartsAfterBE is set, don't exit until BE has been reached
            if(m_settings.RRM_TrailStartsAfterBE && !m_excursion.be_reached)
               break;

            // Guard: only evaluate once per new bar (shift=1 decision, not every tick)
            datetime bar1_time = iTime(m_symbol, PERIOD_CURRENT, 1);
            if(bar1_time == m_trail_ema_last_bar)
               break;   // already evaluated this bar
            ...
            // Evaluated once per bar close (shift=1), consistent with TS evaluation logic.

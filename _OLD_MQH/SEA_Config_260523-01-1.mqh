...
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_Ema1                  = 9;      // EMA1: trailing exit reference
input int         Inp_TI_Ema2                  = 50;     // EMA2: primary bounce level
input int         Inp_TI_Ema3                  = 89;     // EMA3: intermediate structure
input int         Inp_TI_Ema4                  = 200;    // EMA4: major trend anchor (bias slow)

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Signal Architecture       ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_BlockUnorderedPhase   = true;   // Block trades in unordered EMA phase
input bool        Inp_TI_RequireMinPhaseConfirm = true;  // Require phase confirmation bars
input bool        Inp_TI_Emerging_AllowStrong  = false;  // Allow strong-layer trades in EM phase
input bool        Inp_TI_CloseOnReverse        = false;  // Close position on bias reversal

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Layer (Pullback Detection)║";
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_LayerBaselineLookback = 34;     // Layer baseline lookback bars
input double      Inp_TI_LayerPullbackRatio    = 0.5;    // Pullback depth ratio (0=none, 1=full retrace)
input double      Inp_TI_LayerRecoveryRatio    = 0.3;    // Recovery ratio required after pullback
input double      Inp_TI_LayerFlatRatio        = 0.15;   // Flat/sideways tolerance ratio

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — HTF Confirmation          ║";
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_MTF_EMA_Fast          = 50;     // HTF EMA fast (institutional standard)
input int         Inp_TI_MTF_EMA_Slow          = 200;    // HTF EMA slow (institutional standard)

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Exit Strategy             ║";
input group "╚════════════════════════════════════════════════════════╝";
input ETrailingMode   Inp_TI_TrailMode         = TRAIL_EMA;          // Trail mode (EMA9 exit is TI standard)
input int             Inp_TI_TrailEMA_Shift    = 1;      // Trail EMA shift: 1=tight, 2=one-bar cushion
input ETrailTrigger   Inp_TI_TrailTrigger      = TRIGGER_BREAKEVEN;  // When to start trailing
input double          Inp_TI_TrailStepPips     = 5.0;    // Trail step size (pips)
input double          Inp_TI_TrailProfitPercent = 2.0;   // Trail lock profit % (PROFIT_PERCENT trigger)
input ESLMode         Inp_TI_SLMode            = SL_MODE_SWING;      // SL placement mode
input ETPMode         Inp_TI_TPMode            = TP_MODE_RR;         // TP mode
input double          Inp_TI_RRRatio           = 2.0;    // Risk:Reward ratio
input EBEMode         Inp_TI_BE_Mode           = BE_MODE_R_MULTIPLE; // Breakeven mode
input double          Inp_TI_BE_RMultiple      = 1.0;    // Breakeven trigger at N×R profit

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — EMA Fan Filter            ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_EmaFanFilterEnabled   = true;   // Block overextended EMA fan entries
input double      Inp_TI_EmaFanBase_M1M5       = 50.0;   // Fan pip threshold base: M1–M5
input double      Inp_TI_EmaFanBase_M6M30      = 80.0;   // Fan pip threshold base: M6–M30
input double      Inp_TI_EmaFanBase_H1         = 120.0;  // Fan pip threshold base: H1
input double      Inp_TI_EmaFanBase_H2H4       = 200.0;  // Fan pip threshold base: H2–H4
input double      Inp_TI_EmaFanBase_H4Plus     = 350.0;  // Fan pip threshold base: H4+

input group "╔════════════════════════════════════════════════════════╗";
input group "║   📐 TOPINVESTOR: STANDARD — Drawdown Protection       ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_TI_EnableDDProtection    = true;   // Enable drawdown protection
input int         Inp_TI_MaxConsecutiveLosses  = 4;      // Max consecutive losses before pause
input int         Inp_TI_MaxTradesPerDay       = 0;      // Max trades per day (0=unlimited)
input double      Inp_TI_MaxDailyDrawdownPct   = 2.0;    // Max daily drawdown % before pause
...
input group "╚════════════════════════════════════════════════════════╝";
input ETIProfile  Inp_TI_Profile               = TI_MODERATE; // Profile: Conservative(4v)|Moderate(7v)|Full(10v)
input int         Inp_TI_MinBarsAfterClose     = 3;      // Post-trade cooldown bars (0=off)
input bool        Inp_TI_PhaseAllowEM          = true;   // Allow Emerging phase (false=Trending-Market only)
...
input group "╚════════════════════════════════════════════════════════╝";
input double      Inp_TI_Psar_Step             = 0.02;   // PSAR acceleration step
input double      Inp_TI_Psar_Max              = 0.2;    // PSAR max acceleration
input int         Inp_TI_ADX_Period            = 14;     // ADX period
input double      Inp_TI_ADX_Percentile        = 50.0;   // ADX dynamic percentile threshold
input int         Inp_TI_ADX_Lookback          = 100;    // ADX percentile lookback bars
input double      Inp_TI_ADX_Threshold_Accum   = 12.0;   // ADX min in Accumulation phase
input double      Inp_TI_ADX_Threshold_Trend   = 25.0;   // ADX min in Trending phase
input double      Inp_TI_ADX_Threshold_Dist    = 18.0;   // ADX min in Distribution phase
input int         Inp_TI_CandleBody_AvgPeriod  = 10;     // CandleBody average period (bars)
input double      Inp_TI_CandleBody_MaxMult    = 2.5;    // CandleBody max spike multiplier
...
input group "╚════════════════════════════════════════════════════════╝";
input int         Inp_TI_MACD_Fast             = 12;     // MACD fast EMA period
input int         Inp_TI_MACD_Slow             = 26;     // MACD slow EMA period
input int         Inp_TI_MACD_Signal           = 9;      // MACD signal period
input int         Inp_TI_MACD_FreshBars        = 5;      // MACD: max bars since histogram cross
input double      Inp_TI_MACD_SlopeMin         = 0.00001; // MACD: minimum histogram slope
input int         Inp_TI_CCI_Period            = 14;     // CCI period
input int         Inp_TI_BB_Period             = 20;     // Bollinger Bands period
input double      Inp_TI_BB_Deviation          = 2.0;    // Bollinger Bands deviation multiplier
...
input group "╚════════════════════════════════════════════════════════╝";
input double      Inp_TI_Fib_MinRetracement    = 0.38;   // Fib min retracement depth (0.0–1.0)
input double      Inp_TI_Fib_MaxRetracement    = 0.618;  // Fib max retracement depth (0.0–1.0)
input int         Inp_TI_Fib_SwingLookback     = 50;     // Fib swing detection lookback bars
input double      Inp_TI_CandleBody_FullRatio  = 0.75;   // Full: min close ratio (body quality gate)

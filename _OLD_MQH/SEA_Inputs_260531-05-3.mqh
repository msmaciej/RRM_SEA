...
input group "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓";
input group "    ▸ TS = B(Bias)·P(Phase)·F(Filters)·L(Layer)·I(Indicators), then CG veto.  Steps set B/P/L · F = EMA-fan + DPI pre-filters · I = the INDICATORS section below";
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 1 — B (Bias): Major Trend Direction";
input group "╚════════════════════════════════════════════════════════╝";
input EBiasMode   Inp_CUSTOM_BiasMode              = BIAS_4EMA;      // Custom: Bias Mode: Manual, 2-EMA, 4-EMA
...
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 2 — EMA ribbon (substrate for B / P / F / L)";
input group "╚════════════════════════════════════════════════════════╝";
...
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 3 — Entry strategy selector";
input group "╚════════════════════════════════════════════════════════╝";
input EAutoStrategy  Inp_CUSTOM_AutoStrat          = STRAT_4EMA_LAYER;  // Custom: STRATegy
input bool        Inp_CUSTOM_CloseOnReverse        = false;          // Custom: Close On Reverse
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 3b — P (Phase) · L (Layer) · CG · VPRR toggles";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_PhaseDetectionEnabled = true;          // Custom: [PH] Enable market-phase detection (TRENDING/EMERGING/UNORDERED)
input bool        Inp_CUSTOM_BlockUnorderedPhase   = true;           // Custom: [PH] Block trades while phase is UNORDERED
input bool        Inp_CUSTOM_BlockEmergingPhase    = false;          // Custom: [PH] Block trades while phase is EMERGING
input bool        Inp_CUSTOM_EnableLayerDetection  = true;          // Custom: [LY] Enable EMA-layer detection (L1/L2/L3)
input bool        Inp_CUSTOM_LayerPullbackEnabled  = false;          // Custom: [LY] Enable layer pullback-recovery state machine
input bool        Inp_CUSTOM_ClimaxGuard_Enabled   = true;           // Custom: [CG] Enable climax/exhaustion guard
input bool        Inp_CUSTOM_VPRR_Enabled          = false;          // Custom: [VP] Enable Volume Pullback-Recovery Ratio voter
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 4 — L: Candle Close (layer-aware BC gate)";
input group "╚════════════════════════════════════════════════════════╝";
input EBarCloseMode  Inp_CUSTOM_BarClose_Mode      = BC_LAYER_AWARE; // Custom: [CC] CClose Mode: DISABLED/FIXED_EMA/LAYER_AWARE/BIAS_FAST
input EEmaRole    Inp_CUSTOM_BarClose_DefaultEMA   = ROLE_EMA1;      // Custom: [CC] CClose in FIXED mode: EMA1, EMA2, EMA3, EMA4
input bool        Inp_CUSTOM_BarClose_Enabled      = true;           // Custom: [CC] CClose Enable
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP 5 — L: Pullback Gate";
input group "╚════════════════════════════════════════════════════════╝";
...
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🔧 STEP X — Exit trail (post-entry; not a TS factor)";
input group "╚════════════════════════════════════════════════════════╝";
...
input group " ";
input group "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓";
input group "    ⚠️  PRESET_CUSTOM — INDICATORS (= I factor · voters)";
input group "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓";
input group "╔════════════════════════════════════════════════════════╗";
...
input group "╚════════════════════════════════════════════════════════╝";
input EMacdVoteMode  Inp_CUSTOM_Ind_Macd_Mode      = MACD_ZERO_AND_HIST; // Ind [MACD]: Mode
input bool        Inp_CUSTOM_Ind_Macd_Enabled      = false;          // Ind [MACD]: Enable MACD
input bool        Inp_CUSTOM_Ind_Macd_RequireSlope       = false;    // Ind [MACD]: Require Slope
input bool        Inp_CUSTOM_Ind_Macd_RequireDivergence  = false;    // Ind [MACD]: Require Divergence
input bool        Inp_CUSTOM_Ind_Macd_RequireHook        = false;    // Ind [MACD]: Require Histogram Flip
input int         Inp_CUSTOM_Ind_Macd_Fast         = 8;              // Ind [MACD]: Fast EMA period
input int         Inp_CUSTOM_Ind_Macd_Slow         = 13;             // Ind [MACD]: Slow EMA period
input int         Inp_CUSTOM_Ind_Macd_Sig          = 5;              // Ind [MACD]: Signal SMA period
input int         Inp_CUSTOM_Ind_Macd_FreshBars    = 3;              // Ind [MACD]: Fresh Bars validity
input double      Inp_CUSTOM_Ind_Macd_SlopeMin     = 0.00001;       // Ind [MACD]: Min slope change per bar
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 MFI (Money Flow Index - Oscillator Buying Selling Pressure)";
input group "╚════════════════════════════════════════════════════════╝";
input EMfiMode    Inp_CUSTOM_Ind_Mfi_Mode          = MFI_ZONE_FILTER; // Ind [MFI]: Mode
input bool        Inp_CUSTOM_Ind_Mfi_Enabled       = false;          // Ind [MFI]: Enable MFI
input int         Inp_CUSTOM_Ind_Mfi_Period        = 14;             // Ind [MFI]: Period
input double      Inp_CUSTOM_Ind_Mfi_Level         = 50.0;           // Ind [MFI]: Threshold/level
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 P123 (Mark Crisp 1-2-3 fractal breakout pattern (see Ross Hook))";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_P123_Enabled      = false;          // Ind [P123]: Enable 1-2-3
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 PSAR (Parabolic Stop and Reverse - Trend-Following Indicator)";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Psar_Enabled      = true;           // Ind [PSAR]: Enable PSAR
input int         Inp_CUSTOM_Ind_PsarFlipDelay     = 10;             // Ind [PSAR]: Flip timer: -1=persistent, 0=flip bar, 1-10=countdown
input double      Inp_CUSTOM_Ind_Psar_Step         = 0.05;           // Ind [PSAR]: Step
input double      Inp_CUSTOM_Ind_Psar_Max          = 0.5;            // Ind [PSAR]: Maximum
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 Ross Hook (Trend Momentum (see to P123))";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_Ross_Enabled      = false;          // Ind [Ross]: Enable Ross Hook
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 RSI (Relative Strength Index - Monentum Oscilator)";
input group "╚════════════════════════════════════════════════════════╝";
input ERsiMode    Inp_CUSTOM_Ind_Rsi_Mode          = RSI_TREND_ABOVE_50; // Ind [RSI]: Mode
input bool        Inp_CUSTOM_Ind_Rsi_Enabled       = false;          // Ind [RSI]: Enable RSI
input int         Inp_CUSTOM_Ind_Rsi_Period        = 14;             // Ind [RSI]: Period
input double      Inp_CUSTOM_Ind_Rsi_OB            = 70.0;           // Ind [RSI]: Overbought
input double      Inp_CUSTOM_Ind_Rsi_OS            = 30.0;           // Ind [RSI]: Oversold
input group "╔════════════════════════════=═══════════════════════════╗";
input group "║   📊 STO (Stochastic Oscillator - Momentum Potential Market Reversals)";
input group "╚════════════════════════════════════════════════════════╝";
input EStochMode  Inp_CUSTOM_Ind_Sto_Mode          = STO_CROSS_SIGNAL;  // Ind [Sto]: Mode
input bool        Inp_CUSTOM_Ind_Sto_Enabled       = false;          // Ind [Sto]: Enable STO
input int         Inp_CUSTOM_Ind_Sto_K             = 5;              // Ind [Sto]: %K period
input int         Inp_CUSTOM_Ind_Sto_D             = 3;              // Ind [Sto]: %D period
input int         Inp_CUSTOM_Ind_Sto_Slow          = 3;              // Ind [Sto]: Slowing
input group "╔════════════════════════════════════════════════════════╗";
input group "║   📊 VRC (Volatility Regime Classifier - Reject Trades in Low Volatility (quiet/choppy markets))";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_CUSTOM_Ind_VRC_Enabled       = false;          // Ind [VRC]: Enable volatility regime
input int         Inp_CUSTOM_Ind_VRC_ATR_Period    = 14;             // Ind [VRC]: VRC-ATR period
input int         Inp_CUSTOM_Ind_VRC_Lookback      = 100;            // Ind [VRC]: Lookback bars for percentile
input double      Inp_CUSTOM_Ind_VRC_LowThreshold  = 33.0;           // Ind [VRC]: Low volatility threshold (percentile)

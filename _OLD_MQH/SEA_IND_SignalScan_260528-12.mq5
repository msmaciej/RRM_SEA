...
input int      PB_RecoveryBars      = 3;           // Bars RECOVERED state stays valid (then resets)
input group "--- RSI---"
...
// ── Per-layer recovery expiry (bar index when RECOVERED first set) ─
int g_layer_s_rec_bar_long  = -1;
int g_layer_m_rec_bar_long  = -1;
int g_layer_w_rec_bar_long  = -1;
int g_layer_s_rec_bar_short = -1;
int g_layer_m_rec_bar_short = -1;
int g_layer_w_rec_bar_short = -1;
double g_buf_ema1[];
double g_buf_ema2[];
double g_buf_ema3[];
double g_buf_ema4[];
                ...
                const int &spread[])
{
   ...
   if(prev_calculated == 0)
   {
      ...
      g_layer_s_rec_bar_long=-1; g_layer_m_rec_bar_long=-1; g_layer_w_rec_bar_long=-1;
      g_layer_s_rec_bar_short=-1; g_layer_m_rec_bar_short=-1; g_layer_w_rec_bar_short=-1;
...
//+------------------------------------------------------------------+
//| RecGet / RecSet — per-layer recovery bar index accessors        |
//| layer: 3=S  2=M  1=W    bias: 1=LONG  -1=SHORT                 |
//+------------------------------------------------------------------+
int RecGet(int layer, int bias)
{
   if(layer==3) return (bias==1) ? g_layer_s_rec_bar_long  : g_layer_s_rec_bar_short;
   if(layer==2) return (bias==1) ? g_layer_m_rec_bar_long  : g_layer_m_rec_bar_short;
                return (bias==1) ? g_layer_w_rec_bar_long  : g_layer_w_rec_bar_short;
}
void RecSet(int layer, int bias, int val)
{
   if(layer==3) { if(bias==1) g_layer_s_rec_bar_long=val;  else g_layer_s_rec_bar_short=val; }
   else if(layer==2) { if(bias==1) g_layer_m_rec_bar_long=val; else g_layer_m_rec_bar_short=val; }
   else              { if(bias==1) g_layer_w_rec_bar_long=val; else g_layer_w_rec_bar_short=val; }
}
...
void Eval(int shift, CSignalEngine &eng, int bias)
{
   ...
   if(any_layer)
   {
      ...
      // Per-layer recovery expiry — use helpers to read/write bias-correct globals
      int rs = RecGet(3, bias), rm = RecGet(2, bias), rw = RecGet(1, bias);

      if(eng.GetLayerSPullbackState()==LAYER_PB_RECOVERED) { if(rs<0) rs=shift; }
      else rs = -1;
      if(eng.GetLayerMPullbackState()==LAYER_PB_RECOVERED) { if(rm<0) rm=shift; }
      else rm = -1;
      if(eng.GetLayerWPullbackState()==LAYER_PB_RECOVERED) { if(rw<0) rw=shift; }
      else rw = -1;

      RecSet(3,bias,rs); RecSet(2,bias,rm); RecSet(1,bias,rw);

      // Expire layers whose recovery window has elapsed
      if(rs >= 0 && (rs - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(3); rs=-1; RecSet(3,bias,-1); }
      if(rm >= 0 && (rm - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(2); rm=-1; RecSet(2,bias,-1); }
      if(rw >= 0 && (rw - shift) >= PB_RecoveryBars) { eng.Scanner_ResetLayerAfterFire(1); rw=-1; RecSet(1,bias,-1); }
      ...
      if(!fired_layer && TS_LayerS && eng.GetLayerSPullbackState()==LAYER_PB_RECOVERED)
      {
         ...
         bool stack_ok = (bias==1) ? (e1>e2 && e2>e3 && e3>e4) : (e1<e2 && e2<e3 && e3<e4);
         if(stack_ok){fired_layer=3;fema=e3;}
      }
      if(!fired_layer && TS_LayerM && eng.GetLayerMPullbackState()==LAYER_PB_RECOVERED)
      {
         double e1=EMAv(EMA1,shift), e2=EMAv(EMA2,shift), e3=EMAv(EMA3,shift);
         bool stack_ok = (bias==1) ? (e1>e2 && e2>e3) : (e1<e2 && e2<e3);
         if(stack_ok){fired_layer=2;fema=e2;}
      }
      if(!fired_layer && TS_LayerW && eng.GetLayerWPullbackState()==LAYER_PB_RECOVERED)
      {
         double e1=EMAv(EMA1,shift), e2=EMAv(EMA2,shift);
         bool stack_ok = (bias==1) ? (e1>e2) : (e1<e2);
         if(stack_ok){fired_layer=1;fema=e1;}
      }

...
void ApplyPreset(const EStrategyPreset preset, ST_Settings &cfg)
{
   ...
   if(preset == PRESET_TEST_INDICATOR)
   {
      ...
      // ═══════════════════════════════════════════════════════════
      // 6. EXIT MANAGEMENT: Choose ONE option below
      // ═══════════════════════════════════════════════════════════
      Print("");
      Print("  ┌─────────────────────────────────────────────────────┐");
      Print("  │ EXIT STRATEGY:                                      │");
      Print("  │ • OPTION A (active): Simple fixed SL/TP            │");
      Print("  │ • OPTION B (commented): RRM advanced management    │");
      Print("  │ Uncomment Option B if you want BE + trailing       │");
      Print("  └─────────────────────────────────────────────────────┘");
      Print("");

      // ───────────────────────────────────────────────────────────
      // OPTION A: SIMPLE FIXED EXIT (For Indicator Isolation)
      // 
      // Purpose: Test indicator signal quality without exit complexity
      // Use when: You want clean, predictable results
      // ───────────────────────────────────────────────────────────
      
      cfg.ExitProfile    = EXIT_PROFILE_LEGACY;
      
      // Stop Loss: ATR-based with minimum floor
      cfg.SLMode         = SL_MODE_ATR;
      cfg.SL_Mult        = 1.5;          // 1.5 × ATR
      cfg.SL_FixedPips   = 15.0;         // Minimum 15 pips (fallback if ATR too small)
      cfg.UseATRforSL    = true;         // Use ATR calculation
      
      // Take Profit: Risk-Reward ratio
      cfg.TP_Enabled     = true;
      cfg.TPMode         = TP_MODE_RR;
      cfg.TP_Mult        = 2.0;          // 2:1 reward:risk (exit at 2× SL distance)
      cfg.UseATRforTP    = false;
      ...
      // Risk Management
      cfg.RiskPercent    = 1.0;          // 1% risk per trade
      cfg.CloseOnReverse = false;        // Don't close on opposite signals (for clean testing)
      
      Print("  ✓ EXIT CONFIG: OPTION A (Simple Fixed)");
      Print("    ├─ SL: 1.5×ATR (min 15 pips)");
      Print("    ├─ TP: 2.0 Risk-Reward ratio");
      Print("    ├─ BE: Disabled");
      Print("    ├─ Trailing: Disabled");
      Print("    └─ Risk: 1.0% per trade");
      
      // ───────────────────────────────────────────────────────────
      // OPTION B: RRM ADVANCED EXIT (For Realistic Performance)
      // 
      // Purpose: Test indicator with full RRM risk management
      // Use when: You want realistic results with BE + trailing
      // 
      // TO ACTIVATE: Uncomment all lines below and comment Option A above
      // ───────────────────────────────────────────────────────────
      /*
      cfg.ExitProfile              = EXIT_PROFILE_RRM;
      
      // Stop Loss: Swing-based (recent structure)
      cfg.SLMode                   = SL_MODE_SWING;
      cfg.SL_Mult                  = 1.5;
      cfg.SL_SwingPipsCushion      = 5.0;     // Add 5 pips beyond swing point
      cfg.SwingLookback            = 10;      // Look back 10 bars for swing
      cfg.UseATRforSL              = false;   // Don't use ATR (use swing structure)
      
      // Take Profit: Risk-Reward ratio
      cfg.TP_Enabled               = true;
      cfg.TPMode                   = TP_MODE_RR;
      cfg.TP_Mult                  = 2.0;     // 2:1 RR
      cfg.UseATRforTP              = false;
      
      // Break Even: Move to BE at 50% progress to TP
      cfg.Use_BE                   = true;
      cfg.BE_Mode                  = BE_MODE_IMMEDIATE;  // Use RRM BE logic
      cfg.RRM_BE_ProgressPct       = 50.0;               // Trigger BE at 50% to TP
      cfg.RRM_BE_BufferPips        = 5.0;                // BE + 5 pips (lock small profit)
      
      // Trailing: PSAR-based (if PSAR enabled) or ATR (for other indicators)
      if(cfg.Ind_Psar_Enabled)
      {
         cfg.TrailMode                = TRAIL_PSAR;
         cfg.RRM_TrailPsarShiftDelay  = 10;              // Wait 10 bars after flip
         cfg.RRM_FreezeTrailOnFlip    = true;            // Freeze SL during flip
         cfg.RRM_TrailStartsAfterBE   = true;            // Only trail after BE hit
         cfg.PSAR_TrailPipsCushion    = 5.0;             // 5 pips cushion from PSAR dot
      }
      else
      {
         cfg.TrailMode                = TRAIL_ATR;
         cfg.Trail_Mult               = 2.0;             // Trail at 2× ATR distance
      ...
      Print("  ✓ EXIT CONFIG: OPTION B (RRM Advanced)");
      Print("    ├─ SL: Swing-based (10-bar lookback + 5 pips)");
      Print("    ├─ TP: 2.0 Risk-Reward ratio");
      Print("    ├─ BE: 50% to TP (lock +5 pips)");
      Print("    ├─ Trailing: ", cfg.TrailMode == TRAIL_PSAR ? "PSAR" : "ATR", " (starts after BE)");
      Print("    └─ Risk: 1.0% per trade");
      */

...
struct ST_VPRRAutoMode
{
   ...
   double min_ratio;
   int    recovery_bars;  // Per-instrument recovery bar count
};

// GetVPRRRecommendedMode — auto-detect VPRR settings for the current symbol.
// Per-instrument MinRatio and RecoveryBars are passed in so each preset can expose
// its own user-facing inputs without duplicating this detection logic.
ST_VPRRAutoMode GetVPRRRecommendedMode(
   double mr_metals,    double mr_indices,   double mr_oil,
   double mr_crypto,    double mr_fx,        double mr_non_fx_tick,
   int    rb_metals,    int    rb_indices,   int    rb_oil,
   int    rb_crypto,    int    rb_fx,
   int    rb_default
)
{
   ...
   if(is_non_fx)
   {
      if(has_real_vol)
      {
         ...
         if(is_metals)       { result.min_ratio = mr_metals;  result.recovery_bars = rb_metals;  }
         else if(is_indices) { result.min_ratio = mr_indices; result.recovery_bars = rb_indices; }
         else if(is_oil)     { result.min_ratio = mr_oil;     result.recovery_bars = rb_oil;     }
         else if(is_crypto)  { result.min_ratio = mr_crypto;  result.recovery_bars = rb_crypto;  }
         PrintFormat("📊 [VPRR AUTO] %s: real vol → ENABLED (REAL, MinRatio=%.2f, RecBars=%d)",
                     sym, result.min_ratio, result.recovery_bars);
      }
      else if(has_tick_vol)
      {
         result.enabled       = true;
         result.volume_type   = (int)VPRR_VOL_TICK;
         result.min_ratio     = mr_non_fx_tick;
         result.recovery_bars = rb_default;
         PrintFormat("📊 [VPRR AUTO] %s: real vol unavailable → ENABLED (TICK fallback, MinRatio=%.2f, RecBars=%d)",
                     sym, result.min_ratio, result.recovery_bars);
      }
      else
   ...
   else
   {
      if(has_tick_vol)
      {
         result.enabled       = true;
         result.volume_type   = (int)VPRR_VOL_TICK;
         result.min_ratio     = mr_fx;
         result.recovery_bars = rb_fx;
         PrintFormat("📊 [VPRR AUTO] %s: FX tick vol → ENABLED (TICK, MinRatio=%.2f, RecBars=%d)",
                     sym, result.min_ratio, result.recovery_bars);
      }
      else
...
void ApplyPreset(const EStrategyPreset preset, ST_Settings &cfg)
{
   ...
   if(preset == PRESET_RRM)
   {
      ...
      if(Inp_RRM_VPRR_AutoEnable)
      {
         ST_VPRRAutoMode vprr_auto = GetVPRRRecommendedMode(
            Inp_RRM_VPRR_MinRatio_Metals,    Inp_RRM_VPRR_MinRatio_Indices,
            Inp_RRM_VPRR_MinRatio_Oil,       Inp_RRM_VPRR_MinRatio_Crypto,
            Inp_RRM_VPRR_MinRatio_FX,        Inp_RRM_VPRR_MinRatio_NonFXTick,
            Inp_RRM_VPRR_RecBars_Metals,     Inp_RRM_VPRR_RecBars_Indices,
            Inp_RRM_VPRR_RecBars_Oil,        Inp_RRM_VPRR_RecBars_Crypto,
            Inp_RRM_VPRR_RecBars_FX,         Inp_RRM_VPRR_RecoveryBars
         );
      ...
      else
      {
         ...
         cfg.VPRR_MinRatio        = MathMax(0.1, Inp_RRM_VPRR_MinRatio_FX);
   ...
   if(preset == PRESET_RRM_ORG)
   {
      ...
      {
         ST_VPRRAutoMode vprr_auto = GetVPRRRecommendedMode(
            Inp_RRM_ORG_VPRR_MinRatio_Metals,   Inp_RRM_ORG_VPRR_MinRatio_Indices,
            Inp_RRM_ORG_VPRR_MinRatio_Oil,       Inp_RRM_ORG_VPRR_MinRatio_Crypto,
            Inp_RRM_ORG_VPRR_MinRatio_FX,        Inp_RRM_ORG_VPRR_MinRatio_NonFXTick,
            Inp_RRM_ORG_VPRR_RecBars_Metals,     Inp_RRM_ORG_VPRR_RecBars_Indices,
            Inp_RRM_ORG_VPRR_RecBars_Oil,        Inp_RRM_ORG_VPRR_RecBars_Crypto,
            Inp_RRM_ORG_VPRR_RecBars_FX,         Inp_RRM_ORG_VPRR_RecoveryBars
         );
         cfg.VPRR_Enabled     = vprr_auto.enabled;
         cfg.VPRR_VolumeType  = vprr_auto.volume_type;
         cfg.VPRR_MinRatio    = vprr_auto.min_ratio;
      ...
      else
      {
         ...
         cfg.VPRR_MinRatio        = MathMax(0.1, Inp_RRM_ORG_VPRR_MinRatio_FX);  // Manual: use FX input as general override
   ...
   if(preset == PRESET_TOPINVESTOR)
   {
      ...
      {
         ST_VPRRAutoMode vprr_auto = GetVPRRRecommendedMode(
            Inp_TI_VPRR_MinRatio_Metals,     Inp_TI_VPRR_MinRatio_Indices,
            Inp_TI_VPRR_MinRatio_Oil,        Inp_TI_VPRR_MinRatio_Crypto,
            Inp_TI_VPRR_MinRatio_FX,         Inp_TI_VPRR_MinRatio_NonFXTick,
            Inp_TI_VPRR_RecBars_Metals,      Inp_TI_VPRR_RecBars_Indices,
            Inp_TI_VPRR_RecBars_Oil,         Inp_TI_VPRR_RecBars_Crypto,
            Inp_TI_VPRR_RecBars_FX,          Inp_TI_VPRR_RecoveryBars
         );
         cfg.VPRR_Enabled         = vprr_auto.enabled;
         cfg.VPRR_VolumeType      = vprr_auto.volume_type;
         cfg.VPRR_MinRatio        = vprr_auto.min_ratio;
      ...
      else
      {
         ...
         cfg.VPRR_MinRatio        = MathMax(0.1, Inp_TI_VPRR_MinRatio_FX);

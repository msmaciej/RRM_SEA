...
input group "╚════════════════════════════════════════════════════════╝";
input double          Inp_RRM_VPRR_MinRatio_Metals   = 1.0;           // RRM VPRR Gold/Silver: MinRatio (1.0–1.2 M15; 0.9–1.1 H1)
input int             Inp_RRM_VPRR_RecBars_Metals    = 3;             // RRM VPRR Gold/Silver: RecoveryBars (3 on M15; 2 on H1)
input double          Inp_RRM_VPRR_MinRatio_Indices  = 1.1;           // RRM VPRR Indices: MinRatio (1.1–1.3; higher = stricter institutional confirmation)
input int             Inp_RRM_VPRR_RecBars_Indices   = 3;             // RRM VPRR Indices: RecoveryBars (3–4)
input double          Inp_RRM_VPRR_MinRatio_Oil      = 0.9;           // RRM VPRR Oil (WTI/Brent): MinRatio (0.9–1.0)
input int             Inp_RRM_VPRR_RecBars_Oil       = 3;             // RRM VPRR Oil: RecoveryBars (2–3)
input double          Inp_RRM_VPRR_MinRatio_Crypto   = 0.7;           // RRM VPRR Crypto (BTC/ETH): MinRatio (0.7–0.8)
input int             Inp_RRM_VPRR_RecBars_Crypto    = 2;             // RRM VPRR Crypto: RecoveryBars (2)
input double          Inp_RRM_VPRR_MinRatio_FX       = 0.7;           // RRM VPRR FX: MinRatio (0.6–0.7; tick vol approximation)
input int             Inp_RRM_VPRR_RecBars_FX        = 3;             // RRM VPRR FX: RecoveryBars
input double          Inp_RRM_VPRR_MinRatio_NonFXTick = 0.8;          // RRM VPRR non-FX tick fallback: MinRatio
input group "╔════════════════════════════════════════════════════════╗";
...
input double          Inp_RRM_ORG_VPRR_MinRatio_Metals   = 1.0;      // RRM ORG VPRR Gold/Silver: MinRatio (1.0–1.2 M15; 0.9–1.1 H1)
input int             Inp_RRM_ORG_VPRR_RecBars_Metals    = 3;        // RRM ORG VPRR Gold/Silver: RecoveryBars (3 on M15; 2 on H1)
// Indices: futures volume from CME/Eurex — very clean institutional signal
// Session: US indices NY only (14:30–21:00 UTC). DAX Frankfurt (07:00–15:30 UTC). Never pre/post-market.
// MinRatio 1.1–1.3. RecoveryBars 3–4.
input double          Inp_RRM_ORG_VPRR_MinRatio_Indices  = 1.1;      // RRM ORG VPRR Indices: MinRatio (1.1–1.3; higher = stricter institutional confirmation)
input int             Inp_RRM_ORG_VPRR_RecBars_Indices   = 3;        // RRM ORG VPRR Indices: RecoveryBars (3–4)
// Oil: NYMEX/ICE futures — real volume available, moderate institutional density
// Session: NY open + London afternoon. Avoid weekends and thin overnight sessions.
// MinRatio 0.9–1.0. RecoveryBars 2–3.
input double          Inp_RRM_ORG_VPRR_MinRatio_Oil      = 0.9;      // RRM ORG VPRR Oil (WTI/Brent): MinRatio (0.9–1.0)
input int             Inp_RRM_ORG_VPRR_RecBars_Oil       = 3;        // RRM ORG VPRR Oil: RecoveryBars (2–3)
// Crypto: exchange volume available but highly retail-driven — erratic spikes
// MinRatio 0.7–0.8 (looser; retail noise). RecoveryBars 2.
input double          Inp_RRM_ORG_VPRR_MinRatio_Crypto   = 0.7;      // RRM ORG VPRR Crypto (BTC/ETH): MinRatio (0.7–0.8; retail noise, looser threshold)
input int             Inp_RRM_ORG_VPRR_RecBars_Crypto    = 2;        // RRM ORG VPRR Crypto: RecoveryBars (2)
// FX: tick volume only — proxy, not true institutional volume
// MinRatio 0.6–0.7 (approximation). RecoveryBars 3.
input double          Inp_RRM_ORG_VPRR_MinRatio_FX       = 0.7;      // RRM ORG VPRR FX (majors/minors): MinRatio (0.6–0.7; tick vol approximation)
input int             Inp_RRM_ORG_VPRR_RecBars_FX        = 3;        // RRM ORG VPRR FX: RecoveryBars
// Non-FX tick fallback: non-FX instrument where broker doesn't pipe real volume
input double          Inp_RRM_ORG_VPRR_MinRatio_NonFXTick = 0.8;     // RRM ORG VPRR non-FX tick fallback: MinRatio (0.8; some brokers omit real vol even for metals)
input group "╔════════════════════════════════════════════════════════╗";
...
input group "╚════════════════════════════════════════════════════════╝";
input double          Inp_TI_VPRR_MinRatio_Metals    = 1.0;           // TI VPRR Gold/Silver: MinRatio (1.0–1.2 M15; 0.9–1.1 H1)
input int             Inp_TI_VPRR_RecBars_Metals     = 3;             // TI VPRR Gold/Silver: RecoveryBars
input double          Inp_TI_VPRR_MinRatio_Indices   = 1.1;           // TI VPRR Indices: MinRatio (1.1–1.3)
input int             Inp_TI_VPRR_RecBars_Indices    = 3;             // TI VPRR Indices: RecoveryBars
...
input double          Inp_TI_VPRR_MinRatio_FX        = 0.7;           // TI VPRR FX: MinRatio (0.6–0.7; tick vol approximation)

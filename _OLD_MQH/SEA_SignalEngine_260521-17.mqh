...
public:
   ...
   EMarketPhase DetectMarketPhase(const int shift = 1)
   {
      ...
      // Stack ordering: are EMAs in a directional sequence?
      bool stack_bullish = (ema2 > ema3 && ema3 > ema4);
      bool stack_bearish = (ema4 > ema3 && ema3 > ema2);
      
      // Slope check: EMA2 (fastest of the trio) must be moving in the direction of the stack.
      // Without this, the phase detection lags badly during reversals — EMAs remain stacked
      // from the prior trend while price and the fast EMA slope have already reversed.
      // FIX (2026-05-21): Previously only checked stack order, which gave TRENDING_UP
      // during a clear downtrend on Gold M1 because EMA50/89/200 hadn't crossed yet.
      double ema2_prev = GetMAVal(h_ema2, shift + 1, 0);
      bool ema2_slope_up   = (ema2_prev != EMPTY_VALUE && ema2 > ema2_prev);
      bool ema2_slope_down = (ema2_prev != EMPTY_VALUE && ema2 < ema2_prev);

      // TM: perfect ascending/descending stack WITH slope confirmation
      if(stack_bullish && ema2_slope_up)  return PHASE_TRENDING_UP;
      if(stack_bearish && ema2_slope_down) return PHASE_TRENDING_DN;
      
      // Stack is ordered but slope contradicts → downgrade to EMERGING
      // (trend structure exists but momentum is fading or reversing)
      if(stack_bullish) return PHASE_EMERGING_UP;
      if(stack_bearish) return PHASE_EMERGING_DN;

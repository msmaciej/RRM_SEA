...
private:
   ...
   void RRM_ManageStrictNoATR(ulong ticket) {
      ...
      if(m_settings.TrailMode == TRAIL_EMA) {
         ...
         if(ema_period <= 0) ema_period = (int)m_settings.P_Ema1;
         if(ema_period <= 0) ema_period = 5;
         ...
         // Use dedicated EMA cushion if set; otherwise fall back to instrument-scaled PSAR cushion
         double ema_cushion_pips = (m_settings.TrailEMA_CushionPips > 0.0)
            ? m_settings.TrailEMA_CushionPips

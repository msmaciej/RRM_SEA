...
struct ST_Settings
{
   ...
   int             NewsPre;
   int             NewsPost;
   bool            Filter_MTF_Enable;         // MTF filter enabled
   ENUM_TIMEFRAMES Filter_MTF_TF1;            // MTF confirmation timeframe 1
   ENUM_TIMEFRAMES Filter_MTF_TF2;            // MTF confirmation timeframe 2 (PERIOD_CURRENT = single TF mode)
   int             Filter_MTF_EMA_Fast;       // MTF fast EMA period
   int             Filter_MTF_EMA_Slow;       // MTF slow EMA period
   bool            Filter_MTF_RequirePhase;   // MTF: require trending phase
   bool            Filter_MTF_StrictAlignment;// MTF: strict all-TF alignment
...
input group "╔════════════════════════════════════════════════════════╗";
input group "║   🚫 FILTER: MTF (Multi-Timeframe Confirmation)        ║";
input group "╚════════════════════════════════════════════════════════╝";
input bool        Inp_Filter_MTF_Enable            = false;       // MTF: Enable filter
input ENUM_TIMEFRAMES Inp_Filter_MTF_TF1           = PERIOD_M5;   // MTF: TF1
input ENUM_TIMEFRAMES Inp_Filter_MTF_TF2           = PERIOD_M15;  // MTF: TF2 (PERIOD_CURRENT = single-TF mode)
input int         Inp_Filter_MTF_EMA_Fast          = 20;          // MTF: Fast EMA
input int         Inp_Filter_MTF_EMA_Slow          = 50;          // MTF: Slow EMA
input bool        Inp_Filter_MTF_RequirePhase      = true;        // MTF: Require trending phase
input bool        Inp_Filter_MTF_StrictAlignment   = true;        // MTF: Strict (all agree) vs Flexible

// ╔════════════════════════════════════════════════════════════════╗
// ║ DEPRECATED: Old HTF filter (replaced by MTF global filter)    ║
// ║ Legacy inputs kept for backward compatibility migration        ║
// ╚════════════════════════════════════════════════════════════════╝
input bool        Inp_Filter_UseHTF                = false;       // DEPRECATED: Use Inp_Filter_MTF_Enable
input ENUM_TIMEFRAMES Inp_Filter_HtfPeriod         = PERIOD_H4;   // DEPRECATED: Use Inp_Filter_MTF_TF1
input int         Inp_Filter_HtfEmaPeriod          = 89;          // DEPRECATED: Use Inp_Filter_MTF_EMA_Fast/Slow
...
void InitializeConfig()
{
   ...
   Settings.Filter_MTF_Enable          = Inp_Filter_MTF_Enable;
   Settings.Filter_MTF_TF1             = Inp_Filter_MTF_TF1;
   Settings.Filter_MTF_TF2             = Inp_Filter_MTF_TF2;
   Settings.Filter_MTF_EMA_Fast        = MathMax(1, Inp_Filter_MTF_EMA_Fast);
   Settings.Filter_MTF_EMA_Slow        = MathMax(1, Inp_Filter_MTF_EMA_Slow);
   Settings.Filter_MTF_RequirePhase    = Inp_Filter_MTF_RequirePhase;
   Settings.Filter_MTF_StrictAlignment = Inp_Filter_MTF_StrictAlignment;

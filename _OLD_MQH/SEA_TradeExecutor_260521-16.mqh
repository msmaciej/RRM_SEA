...
private:
   ...
   double CalcLotByRisk(double entry_price, double sl_price) {
      ...
      // Sanity check: detect Strategy Tester "profit in pips" mode.
      // In pip mode, MT5 returns tick_value=100.0 for ALL symbols regardless of currency.
      // Real tick_value for Forex ≈ 0.1-10.0 per tick per lot (varies by pair and account currency).
      // tick_value of exactly 100.0 or 1.0 on a Forex/JPY pair is a strong indicator of pip mode.
      if(MathAbs(tick_value - 100.0) < 0.01 || MathAbs(tick_value - 1.0) < 0.001)
      {
         static bool s_pip_mode_warned = false;
         if(!s_pip_mode_warned)
         {
            PrintFormat("⚠️ [LOT CALC] %s tick_value=%.2f — possible Strategy Tester 'profit in pips' mode. "
                        "Lot sizing may be inaccurate. Change Tester Settings to 'profit in deposit currency' for correct risk sizing.",
                        m_symbol, tick_value);
            s_pip_mode_warned = true;
         }
      }
      ...
      if(raw_lot < vol_min && vol_min > 0.0)
      {
         ...
         PrintFormat("⚠️ [LOT CALC] %s raw_lot=%.4f < min_lot=%.2f — clamped to minimum. "
                     "Actual risk: $%.2f (%.2f%%) vs target: $%.2f (%.2f%%)",
                     m_symbol, raw_lot, vol_min,
                     actual_risk_money, actual_risk_pct,
                     risk_money, m_settings.RiskPercent);
      }

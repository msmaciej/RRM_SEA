...
//+------------------------------------------------------------------+
input group "=== STEP 5: Time Window  [0 = no limit] ==="
input datetime DateFrom = 0;   // Scan FROM this date/time  (0 = use BarsBack below)
input datetime DateTo   = 0;   // Scan TO   this date/time  (0 = up to current bar)
input int      BarsBack = 500; // If DateFrom=0: scan this many bars back

//+------------------------------------------------------------------+
//| STEP 6 — Display                                                 |
//+------------------------------------------------------------------+
input group "=== STEP 6: Display ==="
input color           Color_Long  = clrDodgerBlue; // LONG signal line color   (blue)
input color           Color_Short = clrRed;         // SHORT signal line color  (red)
input ENUM_LINE_STYLE LineStyle   = STYLE_DOT;      // Line style
input int             LineWidth   = 1;              // Line width

// Panel style — font/size/spacing from SEA_Config.mqh shared inputs (same as EA)
// Position is independent: bottom-left corner by default, away from EA cockpit.
input group "--- Scanner panel position ---"
input int                Scn_PanelX      = 10;                  // Panel X distance from corner (px)
input int                Scn_PanelY      = 30;                  // Panel Y distance from corner (px)
input ENUM_BASE_CORNER   Scn_PanelCorner = CORNER_LEFT_LOWER;   // Panel corner (default = bottom-left, away from EA cockpit)

#define SCN_PANEL_FONT      Inp_UI_PanelFont
#define SCN_PANEL_FONTSIZE  Inp_UI_PanelFontSize
#define SCN_PANEL_SPACING   Inp_UI_PanelLineSpacingPx

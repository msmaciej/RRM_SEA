...
| EMA read path | `iMA` primary → one handle re-creation attempt → internal EMA with **MT5-identical semantics** (seed on oldest available close, full-history forward recursion, carried forward incrementally). No lookback window, no convergence multiplier — any `period * X` window is a tolerance dial, not the formula, and its error grows with the period. Provenance shown per slot on the cockpit `EMAS:` line (untagged=`iMA`, `(MAN)`=fallback, `(ERR)`=both failed). **Contract: a slot that cannot be produced marks the ribbon invalid and suppresses the phase — never classify on an unusable ribbon.** All four handles validated at init (2026-07; previously only `h_ema1`). — **logic** |
...
| RR | Per `Inp_RRM_ORG_RRRatio`. |
| Trail | Mode per `Inp_RRM_ORG_TrailMode`. **Contract (Oracle: Stop Loss card *"Move Stop Loss Towards Entry"* + manual §IV.B): the stop ratchets behind the PSAR dots from entry, including while still at a loss** — that is what makes a partial loss reachable. Governed by `Inp_RRM_ORG_TrailAllowLossSide` (Oracle value `true`) and `Inp_RRM_ORG_TrailStartsAfterBE` (Oracle value `false`). `Inp_RRM_ORG_TrailLockProfit` remains **immutable in spirit**: the stop is never moved backwards, in loss or in profit (manual §IV.B p.84). Startup journal prints the active shape as `[EXIT SHAPE]`. Full reference: `README_SEA_TRADE_LOGIC.md` §4. |
| BE | Trigger level per `Inp_RRM_ORG_BE_Mode` + `Inp_RRM_ORG_BE_RMultiple` (**contract: 0.7R** — Oracle sets break-even at ~70% of the initial stop). Trigger **sample** per `Inp_RRM_ORG_BE_TriggerSource` — **contract: an intrabar TOUCH** (manual §IV.E: the candle need not close at the level), so `BE_SRC_TICK` is the conformant value. The lock is one-time and latched per ticket. Full reference: `README_SEA_TRADE_LOGIC.md` §4. |

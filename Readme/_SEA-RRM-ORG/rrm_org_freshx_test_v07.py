...
RULE = {"M1": "1min", "M5": "5min", "M15": "15min", "M30": "30min", "H1": "1h", "H4": "4h", "D1": "1D"}
...
def main():
    ...
    ap.add_argument("--horizon", type=int, default=24, help="bars forward for the outcome stats")
    ap.add_argument("--quiet", action="store_true", help="summary + outcome table only")
    ap.add_argument("--from", dest="dfrom", default=None, help="YYYY-MM-DD start (after warm-up is kept automatically)")
    ap.add_argument("--to", dest="dto", default=None, help="YYYY-MM-DD end")
    a = ap.parse_args()

    raw = disc.load_csv(a.csv)
    if a.dfrom: raw = raw[raw.index >= pd.Timestamp(a.dfrom) - pd.Timedelta(days=30)]   # keep 30d warm-up
    if a.dto:   raw = raw[raw.index <  pd.Timestamp(a.dto)]
    def frame(tf):
    ...
    kept = []; verdicts = []          # (i, bias, verdict)
    # ---- vectorised prefilter (same voter maths as evaluate_bar; only saves time) ----
    e2v, e3v, e4v = D.e2.values, D.e3.values, D.e4.values
    bias_v = np.where((e2v>e3v)&(e3v>e4v) | (e2v>e4v)&(e4v>e3v), 1,
             np.where((e4v>e3v)&(e3v>e2v) | (e3v>e4v)&(e4v>e2v), -1, 0))
    shark_v = np.where((e3v>e4v+2e-5)&(e2v<e3v)&(e2v>e4v), 1, np.where((e4v>e3v+2e-5)&(e2v>e3v)&(e2v<e4v), -1, 0))
    dir_v = np.where(bias_v != 0, bias_v, shark_v)
    lng_v = dir_v > 0
    dpi_ok_v  = ((D.dpi.values > 0) == lng_v) & (np.sign(D.dpi.values) == np.sign(D.cci.values))
    psar_ok_v = np.where(lng_v, D.psar.values < D.close.values, D.psar.values > D.close.values)
    rng = np.maximum(D.high.values - D.low.values, 1e-9)
    cb_v = ((D.close.values > D.open.values) == lng_v) & np.where(lng_v, (D.close.values-D.low.values)/rng >= 0.75, (D.high.values-D.close.values)/rng >= 0.75)
    pre_v = (dir_v != 0) & dpi_ok_v & psar_ok_v & cb_v
    i0 = 120
    if a.dfrom: i0 = max(i0, int(D.index.searchsorted(pd.Timestamp(a.dfrom))))
    for i in range(i0, len(D)):
        t = D.index[i]
        if at is not None and t not in at: continue
        if at is None and not pre_v[i]: continue
        ...
        show = (at is not None) or (not a.quiet and len(kept) < a.max_print)
        if not okx:
            stats["freshx"] += 1; verdicts.append((i, bias, "L_FRESHX_STALE"))
            if show: print(line + "  → L_FRESHX_STALE")
            continue
        if dir1 != bias or dir2 != bias:
            stats["mtf_dir"] += 1; verdicts.append((i, bias, "MTF_DIR"))
            if at is not None: print(line + "  → MTF direction")
            continue
        if mtf_scope and not okm:
            stats["mtf_fresh"] += 1; verdicts.append((i, bias, "L_MTF_FRESHX_STALE"))
            if show: print(line + "  → L_MTF_FRESHX_STALE")
            continue
        stats["kept"] += 1; stats["shark"] += int(e["phase"] == "UNO-SHARK")
        verdicts.append((i, bias, "KEPT_SHARK" if e["phase"] == "UNO-SHARK" else "KEPT"))
        kept.append(line)
        if show: print(line + "  → TS=1 (v06)")
    print("\n# SUMMARY  (approximate base; gates exact)")
    for k, v in stats.items(): print(f"  {k:<10}{v}")

    # ---- outcome by verdict: next `horizon` bars, in ATR units of the signal bar ----
    H = a.horizon; hi_v, lo_v, cl_v, atr_v = D.high.values, D.low.values, D.close.values, D.atr.values
    groups = {}
    for i, bias, v in verdicts:
        if i + H >= len(D): continue
        entry, atr = cl_v[i], max(atr_v[i], 1e-9)
        if bias > 0:
            fav = (hi_v[i+1:i+H+1] - entry) / atr; adv = (entry - lo_v[i+1:i+H+1]) / atr
        else:
            fav = (entry - lo_v[i+1:i+H+1]) / atr; adv = (hi_v[i+1:i+H+1] - entry) / atr
        mfe, mae = fav.max(), adv.max()
        f_hit = np.argmax(fav >= 1.0) if (fav >= 1.0).any() else 10**9
        a_hit = np.argmax(adv >= 1.0) if (adv >= 1.0).any() else 10**9
        winfirst = f_hit < a_hit
        g = groups.setdefault(v, [0, 0, 0.0, 0.0]); g[0] += 1; g[1] += int(winfirst); g[2] += mfe; g[3] += mae
    print(f"\n# OUTCOME by verdict — next {H} bars, ATR units (win-first = +1 ATR reached before −1 ATR)")
    print(f"  {'verdict':<20}{'n':>6}{'win-first%':>12}{'mean MFE':>10}{'mean MAE':>10}{'MFE/MAE':>9}")
    for v in ["KEPT", "KEPT_SHARK", "L_FRESHX_STALE", "L_MTF_FRESHX_STALE", "MTF_DIR"]:
        if v in groups:
            n, w, mf, ma = groups[v]
            print(f"  {v:<20}{n:>6}{100*w/n:>11.0f}%{mf/n:>10.2f}{ma/n:>10.2f}{(mf/max(ma,1e-9)):>9.2f}")

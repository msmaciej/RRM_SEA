...
**Reading the three lanes on the iconographic.** A cross is never the entry. 13 × 34 cross → wait for a pullback
to EMA13 (W) or EMA34 (M) → enter on the recovery bar that closes back past EMA5 / EMA13 — W only on the first two
pullbacks, M on the first three. 34 × 89 cross → wait for the deep pullback that reaches EMA89 → enter on the recovery
bar that closes back past EMA34 — any pullback, and the 13 may sit between 34 and 89 (UNO-Shark). In every lane the
voters and filters still have to pass on that bar.

# PRESET_RRM_ORG v05 — Russ Horn RRM (Original, quality-gated)

**Status: IMPLEMENTED & DEFAULT.** Same engine as v01 (`SEA_Presets.mqh`, `if(preset == PRESET_RRM_ORG)`;
shared `EvaluateTS_Breakdown` core in `SEA_SignalEngine.mqh`). v05 is the input set and the four code
...
`_SEA-RRM-ORG/_FX-RRM-ORG-Trading_System_v05.png`.
...
- **Nested-TF freshness (new, "MTF FreshX"):** for S trades TF1 must itself be young — its own 34/89 cross and at most 2 slope-pullbacks since. HTF EMAs changed 21/21 → 34/89.
...
Inp_RRM_ORG_MTF_FreshX_Enabled        = true

# PRESET_RRM_ORG v06 — Russ Horn RRM (Original, quality-gated)

**Status: IMPLEMENTED & DEFAULT.** Same engine as v01 (`SEA_Presets.mqh`, `if(preset == PRESET_RRM_ORG)`;
shared `EvaluateTS_Breakdown` core in `SEA_SignalEngine.mqh`). v06 is the input set and the four code
...
`_SEA-RRM-ORG/_FX-RRM-ORG-Trading_System_v06.png`.
...
- **Nested-TF freshness (new, "MTF FreshX"):** measures whether TF1 is itself young — its own 34/89 cross and at most 2 slope-pullbacks since. HTF EMAs changed 21/21 → 34/89. **Off as a veto by default** (Python test, §12): it is a grade tag for the A-setup, not a filter.
...
| UNO, Shark geometry | ✗ | ✗ | ✓ — S slope pullback-recovery, close past EMA34 |
| UNO, any other order | ✗ | ✗ | ✗ |
...
| Layer | Pullback = slope of | Reference cross (the trend clock) | Cap |
|-------|--------------------|-----------------------------------|-----|
| W | EMA5 (fast EMA of the 5/13 pair) | EMA13 × EMA34 — the ribbon's own 5/13 pair flips on every pullback, so the next-slower pair is the clock | first **2** pullbacks |
| M | EMA13 (fast EMA of the 13/34 pair) | EMA13 × EMA34 — its own pair (a 13/34 cross against bias = UNO = M invalid anyway) | first **3** pullbacks |
| S | EMA34 (fast EMA of the 34/89 pair) | EMA34 × EMA89 — its own pair | unlimited (measured, not gated) |

Walk back from the signal bar to the most recent bias-direction cross of the reference pair; from that bar to now,
count pullback episodes of the layer's fast EMA using **the same slope test the layer machine uses** (pace over
`lookback/4` bars vs baseline pace over `LayerPBLookback`: ratio < `LayerPBFlatRatio`, or slope against bias; a run of
consecutive pullback bars = one episode; the current pullback counts). Fire only while episodes ≤ cap. No cross inside `FreshX_Lookback = 300` bars = stale. Episodes,
...
**Reading the three lanes on the iconographic.** A cross is never the entry. 13 × 34 cross → wait for the W pullback
(EMA5 slope flattens or turns) or the M pullback (EMA13 slope) → enter on the recovery bar that closes back past
EMA5 / EMA13 — W only on the first two pullbacks, M on the first three. 34 × 89 cross → wait for the S pullback (EMA34
slope) → enter on the recovery bar that closes back past EMA34 — any pullback, and the 13 may sit between 34 and 89
(UNO-Shark). In every lane the voters and filters still have to pass on that bar.

**Book "touch" vs engine "slope" — documented design decision.** The Oracle describes the setups as *price touches the
13 / 34 / 89*. A touch cannot be coded objectively (the margin differs by pair, price level and volatility, and a human
reads it with context), so since 2026-07 the engine defines a pullback purely as the layer's fast-EMA slope event and a
recovery as the slope resuming. v02 keeps that definition everywhere: the fresh-cross counter counts slope episodes,
and the UNO-Shark needs the S slope cycle — no price-vs-EMA test anywhere. The lanes above are drawn accordingly.
...
last 2 bars. In the Shark context W/M are hard-blocked (`L_UNO_S_ONLY`).
...
| **MTF** | HTF EMA34 above/below EMA89 and both sloping with it (`MTF_EMA_Fast/Slow = 34/89`, `RequirePhase`). Set per chart to the 4:1 rule: M1→M5+M15 · M5→M15+H1 · M15→H1+H4 · H1→H4+D1 · H4→D1+W1 (report *Multi-Timeframe*: M1 and M5 need two confirming TFs). |
| **MTF FreshX** (new) | The nested read a human makes across screens: an S trade on the chart TF is taken only if TF1 is in the *same young structure* — its own 34/89 cross found within 200 TF1 bars and ≤ 2 slope-pullback episodes of its EMA34 since (same arithmetic as the chart-TF fresh-cross gate, `CheckMtfFreshCross`). TF2 confirms direction only (`ApplyTF2 = false`). Scope `Layers = 3` (S only) by default; 0 applies it to every layer. Reject reason `L_MTF_FRESHX_STALE(tf1:pb3/age140 …)`. Example that motivated it: trading M1, S setup after the M1 34/89 cross, M5 has just had its own 34/89 cross and first pullback-recovery, M15 trending. |
...
- MTF FreshX and the chart-TF fresh-cross gate use the same slope definition and the same "no cross in window = stale" rule; with `MTF_EMA_Fast == MTF_EMA_Slow` (legacy 21/21) MTF FreshX is inert by design.
...
Inp_RRM_ORG_MTF_EMA_Fast / _Slow      = 34 / 89        (was 21/21; needed for MTF FreshX)
Inp_RRM_ORG_MTF_FreshX_Enabled        = false          (grade tag; PY test: as a veto it cut good trades)
Inp_RRM_ORG_MTF_FreshX_Layers         = 3              (0 all · 1 W · 2 M · 3 S)
Inp_RRM_ORG_MTF_FreshX_MaxPullbacks   = 2
Inp_RRM_ORG_MTF_FreshX_MaxBars        = 0
Inp_RRM_ORG_MTF_FreshX_Lookback       = 200            (TF1 bars)
Inp_RRM_ORG_MTF_FreshX_PBLookback     = 55             (TF1 slope baseline, S-layer value)
Inp_RRM_ORG_MTF_FreshX_ApplyTF2       = false
...
Nested : S trades need TF1 young too (34/89 cross, ≤2 pullbacks) — MTF FreshX
...
## 12. Python test of the v06 gates (`_SEA-Scripts/rrm_org_freshx_test.py`)

Exact replicas of the three gates (slope-episode counter, UNO-Shark geometry, TF1/TF2 mapping) on top of the repo's
approximate BASE candidate finder (`rrm_org_discriminator.py`). Outcome proxy: next N bars, in ATR units of the
signal bar; *win-first* = +1 ATR reached before −1 ATR. Runs (v06 session windows on intraday, W off on M5/M1):

| Dataset | Verdict | n | win-first | MFE/MAE |
|---|---|---|---|---|
| **EURUSD H1 2025** (TF1 H4, TF2 D1) | KEPT / KEPT_SHARK | 71 / 15 | 51 % / 60 % | 1.18 / 1.02 |
| | L_FRESHX_STALE | 37 | 41 % | 1.03 |
| | L_MTF_FRESHX_STALE | 24 | 58 % | 2.54 |
| | MTF_DIR reject | 167 | 46 % | 0.72 |
| **EURUSD M15 2025** (TF1 H1, TF2 H4) | KEPT / KEPT_SHARK | 223 / 61 | 55 % / 39 % | 1.27 / 0.86 |
| | L_FRESHX_STALE | 112 | 45 % | 0.92 |
| | L_MTF_FRESHX_STALE | 43 | 53 % | 1.01 |
| | MTF_DIR reject | 604 | 47 % | 0.94 |
| **EURUSD M5 2025** (TF1 M15, TF2 H1) | KEPT / KEPT_SHARK | 413 / 142 | 51 % / 44 % | 1.04 / 1.09 |
| | L_FRESHX_STALE | 63 | 51 % | 1.12 |
| | L_MTF_FRESHX_STALE | 117 | 50 % | 0.81 |
| | MTF_DIR reject | 870 | 50 % | 0.95 |
| **EURUSD M1 Aug 2026** (TF1 M5, TF2 M15) | KEPT / KEPT_SHARK | 195 / 70 | 49 % / 59 % | 1.05 / 1.09 |
| | L_FRESHX_STALE | 34 | 50 % | 1.17 |
| | L_MTF_FRESHX_STALE | 58 | 57 % | 1.50 |
| | MTF_DIR reject | 329 | 45 % | 0.73 |

Read-out: (1) the HTF **direction** vote on 34/89 is the most consistently useful gate — its rejects are the worst
group on H1, M15 and M1; (2) the chart-TF **fresh-cross** gate earns its place on H1 and M15 (rejects 41–45 % vs
51–55 % kept) and is neutral on M5/M1 where W is already off; (3) **MTF FreshX as a veto does not help** — its
rejects perform as well or better than the kept trades on H1 and M1 (older HTF trends keep running, exactly as the
D1/H4 rows of the 100-trade set showed), so v06 keeps it as a measured grade tag with `Enabled = false`;
(4) **UNO-Shark** is strong on H1 and M1, weak on M15 2025 — SignalScan should confirm before trusting the Python
proxy, whose S-layer machine is a 3-bar approximation. Sample sizes for the stale groups are small (24–117).

Commands:
```
python3 rrm_org_freshx_test.py ../Files/EURUSD_H1_202412300000_202512310000.csv --tf H1 --tf1 H4 --tf2 D1 --allow-w --layers all --from 2025-01-01 --to 2026-01-01 --quiet
python3 rrm_org_freshx_test.py ../Files/EURUSD_M15_202501020600_202512312345.csv --tf M15 --tf1 H1 --tf2 H4 --allow-w --layers all --session --quiet
python3 rrm_org_freshx_test.py ../Files/EURUSD_M5_202001020600_202512312355.csv --tf M5 --tf1 M15 --tf2 H1 --layers all --session --from 2025-01-01 --to 2026-01-01 --quiet
python3 rrm_org_freshx_test.py ../Files/EURUSD_M1_202608030000_202608312359.csv --tf M1 --tf1 M5 --tf2 M15 --layers all --session --quiet
python3 rrm_org_freshx_test.py <csv> --at "2026.08.04 12:50"      # explain one bar
python3 rrm_org_freshx_test.py <csv> --session --ideal             # structural windows for eyeballing
```

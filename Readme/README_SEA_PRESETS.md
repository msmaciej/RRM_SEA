...
>
> **Open design decision — VRC/ATR fail-open vs fail-closed.** `GetVolatilityRegime()` deliberately **fail-opens** on an unreadable ATR (`atr <= 0.0 → VOLATILITY_NORMAL`, documented "don't filter when data unavailable"). `Check_ATR` was made fail-**closed** above for determinism. These are the only two non-directional volatility gates and they now disagree on policy. Options: keep VRC glitch-tolerant (a data blip won't halt trading) or flip it to fail-closed for consistency + maximum safety (never trade on unverifiable volatility). No directional risk either way — this is a filtering-policy call for the operator.
>

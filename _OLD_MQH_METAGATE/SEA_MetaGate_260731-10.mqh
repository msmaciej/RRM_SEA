...
//|                                                                    |
//| FILE I/O (NON-common; FILE_COMMON returns INVALID_HANDLE under     |
//| this macOS+Wine tester, so it is not used):                        |
//|  * EVENTS write (collect) — TESTER-ONLY (guarded by MQL5_TESTING):  |
//|      non-common FileOpen -> the per-run agent sandbox               |
//|      Tester\Agent-*\MQL5\Files\  (the tester WIPES this between     |
//|      runs, so it is NOT durable — see archive step below).          |
//|      A LIVE chart writes NOTHING even if Inp_META_LogFeatures is on:|
//|      live only APPLIES the model, never collects.                   |
//|  * MODEL read (gate; works in tester AND live): non-common FileOpen |
//|    from MQL5\Files\ . In the tester the agent is SEEDED from the    |
//|    terminal MQL5\Files\ at run start, so the EA finds the model     |
//|    Python put there; live reads the terminal MQL5\Files\ directly.  |
//|                                                                    |
//| DURABILITY is handled host-side by Scripts/rrm_meta.py, NOT here:  |
//|  * `python3 rrm_meta.py --archive` (after each collect) copies the  |
//|    sandbox TS_events_* into the durable  MetaTrader 5\Files_SEA\    |
//|    events\  (merge + de-dupe). That folder is Python's to read.     |
//|  * `python3 rrm_meta.py` trains from Files_SEA\events\ and writes    |
//|    each MetaModel_* into the terminal MQL5\Files\ (this EA's read    |
//|    location). Do NOT move models into Files_SEA\ — the EA can't      |
//|    read there.                                                      |
//| Files are keyed by preset+symbol+TF, so each chart self-selects.    |
//+------------------------------------------------------------------+
...
void LogTSEvent(int direction, double ref_price, double sl_price)
{
   ...
   // NON-common write: TESTER -> per-run agent sandbox (wiped between runs; made
   // durable host-side by `rrm_meta.py --archive` -> Files_SEA\events\); LIVE ->
   // terminal MQL5\Files directly. See header note.

...
| Recovery (IN-TREND) | Slope-only: fast AND slow layer-EMA slopes back in `bias_dir`. Not a close-vs-EMA test (that is BC). No one-bar recovery. **Fire-on-edge (2026-07):** the layer fires ONLY on the recovery-edge bar (`bars_rec==0`); every later IN-TREND bar is *waiting*. IN-TREND **persists** (no TS=1 consumption reset, no bar-count max-age — both removed) until relapse (counter-`bias_dir` reversal), fast/slow cross, BIAS flip, or sustained-UNO. `Inp_RRM_ORG_LayerRecoveryMaxAgeEnabled` is now inert (dead input, pending removal). |

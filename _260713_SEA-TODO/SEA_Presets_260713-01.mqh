...
void PrintPresetConfiguration(const ST_Settings &cfg, const string preset_name)
{
   ...
   // NEWS-AUDIT 2026-07: print the impact level and the source too — "✓ ±60min" alone hid
   // the fact that the filter was blocking MED+ from a stale CSV in the wrong timezone.
   string news_lvl = (cfg.NewsImpactFilter == NEWS_IMPACT_ALL)       ? "ALL"
                   : (cfg.NewsImpactFilter == NEWS_IMPACT_HIGH_ONLY) ? "HIGH only"
                                                                     : "MED+";
   Print("  News Filter:    ", (cfg.UseNews
         ? ("✓ " + news_lvl + " −" + IntegerToString(cfg.NewsPre) + "/+" + IntegerToString(cfg.NewsPost) + "min | source: MT5 native calendar (server time)")
         : "✗ OFF"));
   Print("");
...
void ApplyPreset(const EStrategyPreset preset, ST_Settings &cfg)
{
   ...
   const double op_EmergencyMarginLevel = cfg.EmergencyMarginLevel;  // STEP2 2026-06: was "Saved for PRESET_TEST exit-profile logic" (TEST removed; kept for Policy A consistency)
   ...
   if(preset == PRESET_RRM_ORG)
   {
      ...
      // close sits mid-range — see GBPUSD H4 trade #2 (05-08 16:00) for the motivating case.
      cfg.CandleBody_MinCloseRatio  = MathMax(0.0, MathMin(1.0, Inp_RRM_ORG_CandleBody_MinCloseRatio));

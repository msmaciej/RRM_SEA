...
FAST,SLOW=20,50; START=None; SLMODE='swing'; ATR_MULT=1.0
...
def signals(d, hsigns, useHTF, useADX, useP):
    ...
    for i in range(SLOW+2,len(d)-1):
        ...
        if useP and not ((ps[i]<cl[i]) if lng else (ps[i]>cl[i])): continue
        out.append((i,lng))
    return out
...
def main():
    ...
    ap.add_argument("--htf",nargs="*",default=[])
    ap.add_argument("--fast",type=int,default=20); ap.add_argument("--slow",type=int,default=50)
    ap.add_argument("--single",action="store_true",help="use only the first HTF")
    ap.add_argument("--start",default=None)
    ap.add_argument("--slmode",default="swing",choices=["swing","atr"])
    ap.add_argument("--atrmult",type=float,default=1.0)
    ap.add_argument("--resample",default=None,help="resample chart to this TF (e.g. M5 from M1)")
    a=ap.parse_args()
    global FAST,SLOW,START,SLMODE,ATR_MULT; FAST,SLOW=a.fast,a.slow; START=pd.Timestamp(a.start) if a.start else None; SLMODE=a.slmode; ATR_MULT=a.atrmult
    df=load(a.csv,a.resample)
    tf=a.resample if a.resample else ("H1" if "_H1_" in a.csv else "M15" if "_M15_" in a.csv else "?")
    if a.htf:
        htf_dfs=[load(h) for h in a.htf]; htflabel=",".join("HTF" for _ in a.htf)
    elif tf=="H1":
        htf_dfs=[load(a.csv,"H4"),load(a.csv,"D1")]; htflabel="H4,D1(resampled)"
    else:
        htf_dfs=[]; htflabel="none"
    if a.single: htf_dfs=htf_dfs[:1]; htflabel=htflabel.split(",")[0]+" (single)"
    ...
    print(f"# EMA {FAST}/{SLOW} | ADX>=median | SL={SLMODE.upper()}"+(f"x{ATR_MULT}" if SLMODE=="atr" else f"({SWING_LB})")+" | cells=meanR (win%/n)\n")
    print("config".ljust(12)+"".join(e.rjust(14) for e in EXITS))
    for label,uH,uA,uP in [("+HTF",1,0,0),("+HTF+ADX",1,1,0),
                            ("+HTF+PSAR",1,0,1),("+HTF+ADX+PSAR",1,1,1)]:
        sigs=signals(d,hsigns,uH,uA,uP); row=label.ljust(14)
        for ex in EXITS:

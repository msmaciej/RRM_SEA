#!/usr/bin/env python3
# gold_map.py — GBPUSD (PIP=0.1, spread $0.30) XEMA analysis with PER-YEAR detail per config.
...
def sigs_of(d,hsigns,fast,slow,useADX=1,useBB=1,usePSAR=1,ci_thr=None,psar_col='psar'):
...
def main():
    ...
    d["psar"]=disc.psar(d,0.08,0.5); d["psar2"]=disc.psar(d,0.02,0.2); d["chop"]=disc.choppiness(d,14)
    ...
    tag=a.csv.split('/')[-1]
    print(f"\n#### GBPUSD {tag}  (netR after 0.8p spread) ####")
    if a.m5:
    ...
    print(f"  PSAR 0.02/0.2    : {fmt(peryear(d,sigs_of(d,hs,13,34,psar_col='psar2')))}")

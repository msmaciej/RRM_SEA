...
| `HOW_TO_RUN.md` | step-by-step ops: verify -> report -> read; what each knob tests; adding a pair |
| `PROMPT_FRAMEWORK_v03.md` | paste into a new chat to run a sweep conversationally (Pair/TF/Span/Spread) |
...
## 4. How to use it (summary — full detail in `HOW_TO_RUN.md`)
...
2. **Report** a pair/TF/span (conversationally via `PROMPT_FRAMEWORK_v03.md`, or):
   ```
...
- Conformance: EURUSD H1 = 20/21 trades, SL + exit-reason exact. Other pairs/TFs use
  ...
  yet; see `HOW_TO_RUN.md` section "add a new pair" to extend the proof.

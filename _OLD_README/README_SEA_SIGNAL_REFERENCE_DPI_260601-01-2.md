...
These definitions are accurate and unchanged from the existing docs:

- **Blue line (LEAD):** fast MACD core = `EMA(Fast) − EMA(Slow)` of close.
- **Red line (FOLLOW):** smoothed signal = EMA of Blue (configurable: EMA5/8/13/21 or Double).
- **Histogram (RIBBON):** `Blue − Red` — the divergence between lead and signal. This is
  what is rendered as the coloured ribbon.
- **GREEN overlay:** `min(|Blue|, |hist|)` shown when Blue and hist are on the **same side**
  of zero. GREEN is a momentum-strength overlay, not a direction.
- **Zero line:** reference for side (above / below).

Reference implementation of the ribbon colour: `SEA_IND_DPI_mc_main.mq5`, the
`hist_wants_yellow` block.

---

## 3. Ribbon colour (the BASE signal)

Colour is decided per bar by the indicator's red/yellow rule (verbatim intent of
`hist_wants_yellow`):

```
if (hist >= 0):  yellow,  UNLESS (CCI enabled AND CCI < 0)  -> then red
else (hist < 0): red,     UNLESS (CCI enabled AND CCI >= 0) -> then yellow
```

So the colour is the **histogram-sign base, with the CCI able to override it**:

| histogram | CCI (when enabled) | ribbon colour |
|-----------|--------------------|---------------|
| hist ≥ 0  | CCI ≥ 0            | **YELLOW**    |
| hist ≥ 0  | CCI < 0            | **RED** (CCI override) |
| hist < 0  | CCI < 0            | **RED**       |
| hist < 0  | CCI ≥ 0            | **YELLOW** (CCI override) |

With CCI disabled, colour follows histogram sign only (hist ≥ 0 → yellow, hist < 0 → red).

**The colour is NOT "histogram sign", and it is NOT "simply CCI sign".** It is the
histogram-sign base with the CCI override applied. This distinction is the whole point:
the override is what produces a *yellow bar inside a red move* (and vice-versa) — the
"reset" referred to in Section 5.

...
input int         Inp_RRM_ORG_PsarFlipDelay_M      = 5;              // RRM ORG PSAR: PSAR Flip delay LayerM override (-99=use global, 0=flip bar, 1-10=window) // BUGFIX A1 2026-07: changed -99→-1. LayerM (EMA2/3) entries confirmed by EMA13/34 structural alignment; persistent dot-position check is sufficient. Allows entries throughout the trend without requiring a fresh flip.
input int         Inp_RRM_ORG_PsarFlipDelay_S      = 5;              // RRM ORG PSAR: PSAR Flip delay LayerS override (-99=use global, 0=flip bar, 1-10=window) // BUGFIX A1 2026-07: changed -99→-1. LayerS (EMA3/4) is the slowest, highest-conviction layer; a time-based flip gate adds no structural filtering value. Persistent mode.

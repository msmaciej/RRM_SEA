...
// Plot 5 — Green Histogram (Green — healthy trend momentum signal) --------
...
double g_HistGreen[];   // Buffer 5: Green healthy trend momentum histogram
                ...
                const int      &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; i--)
   {
      ...
      // ------------------------------------------------------------------
      // Step 7: Green Histogram = healthy trend momentum signal
      // Green PRESENT:  FastLine aligned with MainLine (same sign, nested inside)
      //                 → trend momentum intact
      // Green ABSENT:   FastLine reversed against MainLine
      //                 → overbought/oversold extreme, pullback likely
      // Scale-independent — no threshold needed.
      // ------------------------------------------------------------------
      if(mainHist != 0.0 && nestedHist != 0.0 &&
         (nestedHist * mainHist > 0.0) &&

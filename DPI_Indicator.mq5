...
// Plot 6 — Green Histogram (Green — overbought/oversold exhaustion zone) -
...
double g_HistGreen[];   // Buffer 5: Green overbought/oversold exhaustion histogram
                ...
                const int      &spread[])
{
   ...
   for(int i = rates_total - 2; i >= 0; i--)
   {
      ...
      // ------------------------------------------------------------------
      // Step 7: Green Histogram = overbought/oversold exhaustion zone
      // Drawn when |MainLine| > ThresholdLevel, at the MainLine value
      // Disappears as MainLine returns inside the band → market recovering from extreme
      // ------------------------------------------------------------------
      if(MathAbs(mainLine) > ThresholdLevel)

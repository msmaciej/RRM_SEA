//+------------------------------------------------------------------+
//|                                           SimpleEA_v1-02-010.mq5 |
//|                                  Institutional Trading Solutions |
//|                     News Filter, Time Mgmt, Breakeven, Elastic   |
//+------------------------------------------------------------------+
#property copyright "SimpleEA Redesign v1.02.010"
#property version   "1.02.010"
#property strict
...
int OnInit()
{
   Print("=== SimpleEA v1.02.010 Initializing ===");
   ...
   if(InpUseNewsFilter) Print("News Filter: ON (", ArraySize(g_news_events), " events loaded)");
   else Print("News Filter: OFF");
   
   if(InpUseTimeFilter) Print("Time Filter: ON (", InpStartHour, ":00 - ", InpEndHour, ":00)");
...
void OpenTrade(ENUM_ORDER_TYPE type) {
   ...
   if(trade.PositionOpen(_Symbol, type, lot, price, sl, tp, "SimpleEA v1.02.010")) {

//+------------------------------------------------------------------+
//|                                          SimpleEA_v1-02-012e.mq5 |
//|                                  Institutional Trading Solutions |
//|             GOLDEN MASTER: Reversal Logic | Presets | Formatted  |
//+------------------------------------------------------------------+
#property copyright "SimpleEA Redesign v1.02.12e"
#property version   "102.012" //01.02.012e
#property strict
...
//| INPUT PARAMETERS                                                 |
//+------------------------------------------------------------------+
input group "=== 0. MASTER SETTINGS ==="
input ulong  Inp_MagicNum = 12345;                 // Unique ID for this EA instance
...
int OnInit()
{
   trade.SetExpertMagicNumber(Inp_MagicNum);    // Assign Magic Number to all orders
...
void OnTick()
{
   ...
   // --- REFACTOR START: DIRECT ENTRY BYPASS ---
   // If we are in Reversal Mode, Bias IS the signal. Don't wait for votes.
   if(InpPreset == PRESET_TREND_REVERSAL) {
       if(bias == 1)      ExecuteTrade(ORDER_TYPE_BUY);
       else if(bias == -1) ExecuteTrade(ORDER_TYPE_SELL);
       return; // Exit OnTick here, skip voting entirely
   }
...
void ExecuteTrade(ENUM_ORDER_TYPE signalType) {
   // STEP A: Check for EXISTING position by THIS EA (Using the new helper)
   ulong myTicket = GetMyPosition();
   
   if(myTicket > 0) {
      // Select the trade by its unique Ticket Number to avoid conflicts
      if(PositionSelectByTicket(myTicket)) {
         long posType = PositionGetInteger(POSITION_TYPE);
         
         // 1. Same Direction? -> Do nothing (Let the trend run)
         if((signalType == ORDER_TYPE_BUY && posType == POSITION_TYPE_BUY) ||
            (signalType == ORDER_TYPE_SELL && posType == POSITION_TYPE_SELL)) {
            return;
         }
         
         // 2. Opposite Direction? -> Handle Reversal
         if(Settings.CloseOnReverse) {
            trade.PositionClose(myTicket); // Close only OUR trade
            // After closing, the code proceeds to open the new trade below...
         } else {
            return; // Exit if we are not allowed to reverse
         }
      }
   }

   // STEP B: Open NEW Trade
   ...
   double lot = 0.01; // Fixed lot for testing
   
   // Open the position
   if(trade.PositionOpen(_Symbol, signalType, lot, price, sl, tp, "SimpleEA v1.02.12d")) {
      // Record time to prevent duplicate entry on same bar
...
//+------------------------------------------------------------------+
//| HELPER: Find the Ticket Number for THIS EA's position            |
//| (Fixes the "Fighting" issue by ignoring other trades)            |
//+------------------------------------------------------------------+
ulong GetMyPosition() {
   // Loop backwards through all open positions
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      // Get the ticket for the position at index 'i'
      ulong ticket = PositionGetTicket(i);
      
      // Check if this position belongs to the current symbol AND our Magic Number
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && 
         PositionGetInteger(POSITION_MAGIC) == Inp_MagicNum) {
         return ticket; // Found our specific trade!
      }
   }
   return 0; // No position found for this EA
}
...
//+------------------------------------------------------------------+
void ManageExit() {
   // CRITICAL FIX: Only select trade belonging to THIS instance
   ulong ticket = GetMyPosition();
   
   // If no trade found for this EA, exit immediately
   if(ticket == 0) return; 
   
   // Select the specific ticket to ensure all PositionGet functions work on the correct trade
   if(!PositionSelectByTicket(ticket)) return;
   
   // --- Logic below is now safe because we selected OUR specific ticket ---
   ...
   // B. PSAR Trailing
      ...
      // Safety: Don't trail if PSAR is on the wrong side of price (prevent immediate close)
   ...
   // C. Fractal Trailing
      ...
      for(int i=2; i<20; i++) {
         ...
         if(type == POSITION_TYPE_BUY && down > 0) { 
            new_sl = down; 
            break; 
         }
         if(type == POSITION_TYPE_SELL && up > 0) { 
            new_sl = up; 
            break; 
         }
      }
   ...
   // 3. Modify Order if New SL is Valid
   if(new_sl != 0.0) {
      int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
      ...
      if(type == POSITION_TYPE_BUY) { 
         // Buy: Only move SL UP, and ensure it is below current price
         if(new_sl > current_sl && new_sl < current_price) modify = true;
      } 
      else { 
         // Sell: Only move SL DOWN (or if 0), and ensure it is above current price
...
Summary of Changes:
1. Reversal Logic (CloseOnReverse): If enabled (via Preset or Custom), the EA behaves like the Standard MT5 EA—it flips direction instantly on a signal, catching the full trend.
2. Detailed Reporting: The CSV report now mirrors the Strategy Tester, logging every setting used for the test + the deal history for Excel graphs.
3. Modular Presets: You can switch between "Custom" (Manual Inputs) and "Trend Reversal" (Standard EA Clone) using the first input.
4. Re-Entry Lock: Prevents multiple trades on the same candle.

SimpleEA_v1-02-012e.mq5:

Summary of Changes:
1. Magic Number: Prevents the EA from managing/closing trades that don't belong to it (Fixes "Lots of trades").
2. Ticket Loop: Correctly handles Hedging accounts by finding the specific position ID.
3. Direct Bypass: Eliminates the "Filter Lag" that was causing underperformance against the benchmark.


From a point of view of Experts: Trading Coder, Coding Developer, Coding Senior is code now modular and properly designed?

Coding partner


This is a great question. You have moved the code from "Critically Broken" to "Functionally Stable," but let's look at it through the eyes of three different experts to answer if it is truly "Modular and Properly Designed."
1. The Trading Coder (The Pragmatist)
Verdict: "It is finally safe to run on a live account."
* What they love:
    * Magic Number Isolation: By adding GetMyPosition() and filtering by Magic Number, you fixed the most dangerous bug in algorithmic trading (cross-contamination of orders). This is the industry standard for MT5.
    * Hedging Compatibility: The loop in GetMyPosition() correctly handles the Hedging system (where multiple positions can exist) rather than assuming a simplified Netting environment.
    * Reliability: The check if(myTicket > 0) ensures the EA never tries to modify a ghost trade or a trade that has already closed, preventing those annoying "Order Modify Failed" errors in the journal.
* Their Concern:
    * Execution Speed: While better, the OnTick function still runs a lot of checks (Voting, News, Time) before the new "Direct Entry" bypass. In high-frequency scalping, checking 11 indicators before deciding to skip them is still microseconds of wasted time, though negligible for this strategy.
2. The Coding Developer (The Cleaner)
Verdict: "Much cleaner, but still heavily reliant on Global State."
* What they love:
    * DRY Principle (Don't Repeat Yourself): You extracted the position-finding logic into GetMyPosition(). Previously, you had similar loops inside ExecuteTrade and ManageExit. Now, if you need to change how you find trades, you change it in one place.
    * Single Responsibility: ManageExit now focuses only on exit logic. It delegates the "finding" of the trade to the helper function. This makes the code easier to read and debug.
* Their Concern:
    * Global Variables: The code still relies entirely on the global ST_Settings Settings struct and global indicator handles (g_h_ema1, etc.). If you moved ManageExit to a separate .mqh include file, it would break because it wouldn't know what Settings or g_h_atr are. True modularity usually involves passing these dependencies as arguments or using classes.
3. The Coding Senior (The Architect)
Verdict: "It is robust Procedural Code, but it is not Modular Architecture."
* The Critique:
    * God-Function Anti-Pattern: Your OnTick() is still doing too much. It handles:
        1. Signal Generation (Voting)
        2. Filter Logic (Time/News)
        3. Execution Routing
    * Violation of Open/Closed Principle: If you want to add a new Strategy (e.g., "Gap Trading"), you have to hack the OnTick function with more if statements.
    * The "Hybrid" Smell: The fix we added (The "Direct Entry Bypass") effectively creates two different EAs inside one file:
        * Path A: The Complex Voting EA.
        * Path B: The Simple Trend Reversal EA. While functional, this is technically "Spaghetti Logic" because two distinct behaviors are intertwined in the same control flow.
Final Score
* Safety/Stability: 10/10 (The Magic Number fix is critical).
* Readability: 8/10 (The helper functions make it clear what is happening).
* Modularity: 5/10 (It is improved, but still a monolith file).
Is it ready for production? Yes. For a standalone MT5 EA, this design is standard and acceptable. You do not need to refactor it into full Object-Oriented Classes unless you plan to build a massive library of EAs sharing the same core.
The code is now properly designed for its purpose: It works safely without fighting itself.

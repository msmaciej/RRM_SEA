//+------------------------------------------------------------------+
//|                                          SimpleEA_v1-02-012i.mq5 |
//|                              MJS Institutional Trading Solutions |
//|             GOLDEN MASTER: Modular Architecture | Benchmark Mode |
//+------------------------------------------------------------------+
// SAVE AS UTF-16 LE WITH BOM
#property copyright "SimpleEA Redesign v1.02.12i"
...
enum EManualSide { 
   SIDE_BOTH,     // Trade Long and Short
   SIDE_LONG,     // Long Only 
...
struct ST_Settings {
   ...
   int P_HtfEma; // HTF EMA Period field
   // Voting
   int VoteThreshold;
...
input EAutoStrategy Inp_AutoStrat   = STRAT_PRICE_CROSS; // Auto Strategy (Default to Benchmark)
...
input int    Inp_HtfEmaPeriod       = 89;       // HTF EMA Period (Trend Baseline)
...
int OnInit() {
   ...
   Print("------------------------------------------");
   Print("--- SimpleEA v1.02.12i Configuration ---");
   Print("Symbol: ", _Symbol, " | Magic: ", Inp_MagicNum);
   Print("Mode Used: ", EnumToString(InpPreset));
   Print("Bias Strategy: ", EnumToString(Settings.AutoStrat));
...
void ApplySettings() {
   ...
   if(InpPreset == PRESET_TREND_REVERSAL) {
      ...
      // Force usage of EMA1 (Default 13 or 14) for Price Cross
...
This update (v1.02.012i) is the "Benchmark Fix" release. 
It bridges the gap between your advanced architecture and the raw speed of a standard Moving Average EA.
Here is the summary of what we have added and improved:

1. New Core Logic: STRAT_PRICE_CROSS
* The Problem: Previously, your EA waited for a "Fast MA" to cross a "Slow MA". In fast markets, this signal arrives 1-3 bars after the price has already moved.
* The Fix: We added STRAT_PRICE_CROSS to the EAutoStrategy enum.
* The Behavior: The EA now has the ability to trigger a trade the instant Price closes above or below the MA (just like the standard EA), eliminating entry lag.

2. "Benchmark Mode" Preset
* We updated the PRESET_TREND_REVERSAL to use this new strategy automatically.
* Settings Overrides: When you select this preset, it now forces:
    * Strategy: STRAT_PRICE_CROSS (Instant Entry).
    * Threshold: 1 (No voting delays).
    * Exits: CloseOnReverse = true (Infinite TP/No fixed SL).
    * Result: It now replicates the exact behavior of the "Free MT5 EA" but runs on your superior code base (with better logging and safety).

3. Human-Readable Refactoring
* Signal Engine: I expanded all the compressed "one-liner" logic checks (e.g., Check_RSI, Check_MACD) into proper, multi-line code blocks with clear indentation and comments.
* Maintainability: The code is no longer "code golf." It is designed for a human developer to read, debug, and extend easily.

4. Full Input Control
* Restored the complete list of Inputs (Risk, Time Filters, News, Indicator Periods) to the main file.
* Verified that custom settings are respected when PRESET_CUSTOM is selected, giving you full manual control if you want to tweak the benchmark logic.

You now have a system that can race like a Go-Kart (Standard EA) or drive like a Ferrari (Institutional Grid/Sniper), depending entirely on the Preset you choose.

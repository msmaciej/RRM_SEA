//+------------------------------------------------------------------+
//|                                          SimpleEA_v1-02-012h1.mq5 |
//|                              MJS Institutional Trading Solutions |
//|             GOLDEN MASTER: Modular Architecture | Full Reporting |
//+------------------------------------------------------------------+
// SAVE AS UTF-16 LE WITH BOM
#property copyright "SimpleEA Redesign v1.02.12h1"
#property version   "102.012" // 1.02.012h1 - optimisation
#property strict
...
input EStrategyPreset InpPreset = PRESET_RANGE_GRID; // Select Preset Mode
...
int OnInit() {
   ...
   // D. Restore Print Configuration (Comprehensive Audit)
   Print("------------------------------------------");
   ...
   Print("Reversal Logic: ", Settings.CloseOnReverse ? "ON (Reversal Mode)" : "OFF (Fixed Targets)");
   Print("--- Enabled Indicators ---");
   // FULL Indicator Audit
...
void OnTick() {
   ...
   if(t != g_last_bar_time) {
      ...
      // Management on every tick (Trailing Stop)

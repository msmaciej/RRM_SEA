...
struct ST_Settings {
   ...
   // Execution Logic
   EMaMethod MaType;
   int ma_h_shift;      // Horizontal Shift
   int v_ma_v_shift;    // Vertical/Bar Shift
   // Filters
...
input int           Inp_BarShift    = 1;                    // Vertical Shift: 0=Aggressive (Current Bar), 1=Safe (Closed Bar)
...
input EMaMethod Inp_MaType = METHOD_EMA; // Moving Average Math
input int    Inp_MaHorzShift  = 0;       // Horizontal MA Shift (Indicator Offset)
// Periods
...
int OnInit() {
   ...
   Print("Bar Shift: ", Settings.v_ma_v_shift);
...
void ApplySettings() {
   ...
   Settings.ma_h_shift         = Inp_MaHorzShift;
   Settings.v_ma_v_shift       = Inp_BarShift;
...
2. Vertical/Bar Shift (v_ma_v_shift): In the GetDirection() function, v_shift is extracted from settings.
...
2. Vertical/Bar Shift (v_ma_v_shift)
...
* Vertical/Bar Shift (v_ma_v_shift):
...
Timing	Aggressive (Live Tick)	Inp_BarShift = 0
Horizontal Offset	Visual Shift	Inp_MaHorzShift = [Your Value]
